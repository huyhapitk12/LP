...
The worker restrictions described in [Concurrency and Parallelism](Concurrency-and-Parallelism) are part of the supported public contract: named workers, zero arguments in the current verified build, and `int`/`void` return types.
...
def main():
    ...
    dsa.write_float(x)        # Write float (4 decimal places)
    dsa.write_float_prec(x, 4)    # Write float with 4 decimal places
    dsa.write_float_prec_ln(x, 4) # Write float with precision + newline
...
- `thread.spawn(...)` is not a generic callback runner. It currently requires a named LP worker function with zero arguments returning `int` or `void`.
