...
| `lp file.lp` | Parse, compile, and run an LP file directly (native ASM backend by default). |
...
```bash
lp hello.lp
# or, on Windows / for install verification
lp hello.lp --gcc
```
...
- parse source
- generate LP native assembly by default
- assemble and link a temporary native executable
- run it
- clean up the temporary executable

On Windows, prefer `lp hello.lp --gcc` when you want a predictable install-verification path.
...
This writes LP's native assembly output. It is not the host C compiler's assembly listing.
...
- `lp file.lp` without `--gcc` and `-asm` depend on a working assembler/linker toolchain.
- On Windows, `lp file.lp --gcc` or `lp build file.lp --release --gcc` are the recommended verification commands.
- `--gcc`, `-o`, `-c`, `test`, `profile`, `watch`, `build --gcc`, `package`, and library export depend on a working C compiler.
