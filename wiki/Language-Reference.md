...
- `argv_len()` - Argument count *(internal, not stable public API)*
- `argv_get(i)` - Get argument by index *(internal, not stable public API)*
...
- `spawn(worker)` - Spawn thread with worker function
...
- Worker should have 0 arguments in the current verified build
...
| List literals | ✅ | |
| Dict literals | ✅ | |
| Set literals | ⚠️ | Experimental in the current release |
| Tuple literals | ✅ | |
...
| Feature | Status | Notes |
|---------|--------|-------|
| Async/await | ⚠️ | Syntactic sugar only — synchronous execution, no real async I/O |
| Type unions | ⚠️ | Parsed but no runtime type checking |
| Generic types | ⚠️ | Parsed but no type specialization |
| Set literals | ⚠️ | Experimental in current release |
| List comprehensions | ⚠️ | Numeric-focused; filter conditions partially supported |
| Typed exceptions | ⚠️ | `except SomeType as e:` parsed but runtime is generic |
| `thread.spawn` | ⚠️ | Zero-argument named workers only; no argument passing |
