...
static Symbol *scope_define_obj(Scope *s, const char *name, LpType type,
...
static int is_class_or_subclass(CodeGen *cg, const char *class_name,
  ...
  while (current && target_class) {
    ...
    if (strcmp(current, target_class) == 0)
      return 1;
    sym = scope_lookup(cg->scope, current);
    if (!sym || sym->type != LP_CLASS || !sym->base_class)
      break;
...
static int can_access_member(CodeGen *cg, Symbol *mem) {
  if (!mem || !mem->access)
    return 1;
  if (!cg->current_class || !mem->owner_class)
    return 0;
  if (mem->access == TOK_PRIVATE)
    return strcmp(cg->current_class, mem->owner_class) == 0;
  if (mem->access == TOK_PROTECTED)
    return is_class_or_subclass(cg, cg->current_class, mem->owner_class);
  return 1;
}
...
__attribute__((unused)) static int type_matches_union(const char *type_name,
                                                      const char *union_ann) {
  if (!type_name || !union_ann)
    return 0;
...
static int is_float_array_annotation(const char *ann) {
  if (!ann)
    return 0;
  return (strncmp(ann, "float[", 6) == 0 || strncmp(ann, "f64[", 4) == 0 ||
          strcmp(ann, "float[]") == 0 || strcmp(ann, "f64[]") == 0);
}

/* Parse native array dimensions from type annotation like "int[n+2][m+2]"
 * Returns number of dimensions found, extracts size expressions into sizes
 * array */
static int parse_native_array_dims(const char *ann, char *sizes[4]) {
  if (!ann || !is_native_array_type(ann))
    return 0;
...
static LpType type_from_annotation(CodeGen *cg, const char *ann) {
  ...
  /* i32[] / i32[][] — int32_t native arrays (2x denser than int64, better
   * cache) */
  if (strcmp(ann, "i32[]") == 0)
  ...
  if (strncmp(ann, "f32[", 4) == 0) {
    ...
    for (int i = 0; i < dims; i++)
      if (sizes[i])
        free(sizes[i]);
    return (dims >= 2) ? LP_NATIVE_ARRAY_F32_2D : LP_NATIVE_ARRAY_F32_1D;
  }
  if (strcmp(ann, "i32[][]") == 0)
    return LP_NATIVE_ARRAY_I32_2D;
  if (strncmp(ann, "i32[", 4) == 0) {
  ...
  if (strcmp(ann, "str") == 0)
    return LP_STRING;
  if (strcmp(ann, "bool") == 0)
    return LP_BOOL;
  if (strcmp(ann, "ptr") == 0)
    return LP_PTR;
  if (strcmp(ann, "pool") == 0)
    return LP_POOL;
  if (strcmp(ann, "arena") == 0)
    return LP_ARENA;
  if (strcmp(ann, "val") == 0)
    return LP_VAL;
  if (strcmp(ann, "sqlite_db") == 0)
    return LP_SQLITE_DB;
  if (strcmp(ann, "thread") == 0)
    return LP_THREAD;
  if (strcmp(ann, "lock") == 0)
    return LP_LOCK;
  if (strcmp(ann, "list") == 0)
    return LP_LIST;
  if (strcmp(ann, "dict") == 0)
    return LP_DICT;
  if (cg) {
...
static const char *lp_type_to_c(LpType t) {
  ...
  case LP_NATIVE_ARRAY_1D:
    return "LpIntArray*";
  case LP_NATIVE_ARRAY_2D:
    return "LpIntArray2D*";
  case LP_NATIVE_ARRAY_FLOAT_1D:
    return "LpFloatArray*";
  case LP_NATIVE_ARRAY_FLOAT_2D:
    return "LpFloatArray2D*";
  case LP_NATIVE_ARRAY_I32_1D:
    return "LpI32Array*";
  case LP_NATIVE_ARRAY_I32_2D:
    return "LpI32Array*";
  case LP_NATIVE_ARRAY_F32_1D:
    return "LpF32Array*";
  case LP_NATIVE_ARRAY_F32_2D:
    return "LpF32Array*";
...
static int get_int_literal_value(AstNode *node, int64_t *out) {
  ...
  if (node->type == NODE_INT_LIT) {
    if (out)
      *out = node->int_lit.value;
    return 1;
  }
  if (node->type == NODE_UNARY_OP && node->unary_op.op == TOK_MINUS &&
...
static int is_single_int_repeat_list(CodeGen *cg, AstNode *node,
                                     AstNode **elem_out) {
  AstNode *elem;

  if (!node || node->type != NODE_LIST_EXPR ||
      node->list_expr.elems.count != 1) {
    return 0;
  }
...
static int is_single_float_repeat_list(CodeGen *cg, AstNode *node,
                                       AstNode **elem_out) {
  AstNode *elem;
...
static int match_native_float_repeat_expr(CodeGen *cg, AstNode *node,
                                          AstNode **elem_out,
                                          AstNode **count_out) {
  AstNode *elem = NULL;
  AstNode *count = NULL;
  ...
  if (is_single_float_repeat_list(cg, node->bin_op.left, &elem) &&
      (right_t == LP_INT || right_t == LP_FLOAT)) {
    count = node->bin_op.right;
  } else if (is_single_float_repeat_list(cg, node->bin_op.right, &elem) &&
             (left_t == LP_INT || left_t == LP_FLOAT)) {
...
static int match_native_int_repeat_expr(CodeGen *cg, AstNode *node,
                                        AstNode **elem_out,
                                        AstNode **count_out) {
  AstNode *elem = NULL;
  AstNode *count = NULL;
  ...
  if (is_single_int_repeat_list(cg, node->bin_op.left, &elem) &&
      infer_type(cg, node->bin_op.right) == LP_INT) {
    count = node->bin_op.right;
  } else if (is_single_int_repeat_list(cg, node->bin_op.right, &elem) &&
             infer_type(cg, node->bin_op.left) == LP_INT) {
    count = node->bin_op.left;
  } else {
    return 0;
  }
...
static LpType infer_minmax_call_type(CodeGen *cg, AstNode *node) {
  ...
  if (node->call.args.count == 1) {
    ...
    if (arg_type == LP_NATIVE_ARRAY_1D)
      return LP_INT;
    if (arg_type == LP_NATIVE_ARRAY_I32_1D)
      return LP_INT;
    if (arg_type == LP_NATIVE_ARRAY_I8_1D)
      return LP_INT;
  ...
  for (int i = 0; i < node->call.args.count; i++) {
    LpType arg_type =
        normalize_minmax_scalar_type(infer_type(cg, node->call.args.items[i]));
...
/* Check if every element of a list literal is an integer (literal or
 * unary-minus-int). Used to auto-promote [0,0,1,-1] style direction tables to
 * LpIntArray* instead of LpList*. Only promotes small static literals (<=64
 * elements) to avoid false positives. */
static int list_expr_all_ints(AstNode *node) {
  if (!node || node->type != NODE_LIST_EXPR)
    return 0;
  NodeList *elems = &node->list_expr.elems;
  if (elems->count == 0 || elems->count > 64)
    return 0;
  for (int i = 0; i < elems->count; i++) {
    AstNode *e = elems->items[i];
    if (!e)
      return 0;
    if (e->type == NODE_INT_LIT)
      continue;
    if (e->type == NODE_UNARY_OP && e->unary_op.op == TOK_MINUS &&
        e->unary_op.operand && e->unary_op.operand->type == NODE_INT_LIT)
      continue;
    return 0;
  }
  return 1;
...
static LpType infer_type(CodeGen *cg, AstNode *node) {
  ...
  case NODE_STRING_LIT:
    return LP_STRING;
  case NODE_FSTRING:
    return LP_STRING; /* F-strings always produce strings */
  case NODE_BOOL_LIT:
    return LP_BOOL;
  case NODE_NONE_LIT:
    return LP_VOID;
  ...
  case NODE_UNARY_OP:
    if (node->unary_op.op == TOK_NOT)
      return LP_BOOL;
    if (node->unary_op.op == TOK_BIT_NOT)
      return LP_INT;
  ...
  case NODE_CALL: {
    ...
    if (node->call.func->type == NODE_ATTRIBUTE &&
      ...
      const char *mod_alias =
          node->call.func->attribute.obj->attribute.obj->name_expr.name;
      ...
      if (imp) {
        ...
        if (imp->tier == MOD_TIER1_MATH) {
          if (strcmp(func_name, "factorial") == 0 ||
              strcmp(func_name, "gcd") == 0 || strcmp(func_name, "lcm") == 0)
            return LP_INT;
        ...
        if (imp->tier == MOD_TIER1_SYS) {
          if (strcmp(func_name, "exit") == 0)
            return LP_VOID;
          if (strcmp(func_name, "argv_len") == 0)
            return LP_INT;
          if (strcmp(func_name, "getsizeof") == 0)
            return LP_INT;
          if (strcmp(func_name, "getrecursionlimit") == 0)
            return LP_INT;
          return LP_STRING;
        }
        ...
        if (imp->tier == MOD_TIER1_JSON) {
          if (strcmp(func_name, "loads") == 0 ||
              strcmp(func_name, "parse") == 0)
            return LP_VAL;
        ...
        if (imp->tier == MOD_TIER1_THREAD) {
          ...
          if (strcmp(func_name, "join") == 0)
            return LP_INT;
          if (strcmp(func_name, "lock_init") == 0)
            return LP_LOCK;
          return LP_VOID;
        }
        if (imp->tier == MOD_TIER1_MEMORY) {
          if (strcmp(func_name, "arena_new") == 0)
            return LP_ARENA;
          if (strcmp(func_name, "pool_new") == 0)
            return LP_POOL;
          if (strcmp(func_name, "arena_alloc") == 0)
            return LP_PTR;
          if (strcmp(func_name, "pool_alloc") == 0)
            return LP_PTR;
          if (strcmp(func_name, "cast") == 0)
            return LP_OBJECT;
          return LP_VOID;
        }
        if (imp->tier == MOD_TIER1_PLATFORM) {
          if (strcmp(func_name, "cores") == 0)
            return LP_INT;
        ...
        if (imp->tier == MOD_TIER1_SECURITY) {
          ...
          if (strcmp(func_name, "is_safe_string") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "validate_email") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "validate_numeric") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "validate_alphanumeric") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "validate_url") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "validate_identifier") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "detect_sql_injection") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "detect_xss") == 0)
            return LP_BOOL;
          /* Escape/sanitize functions - return string */
          if (strcmp(func_name, "sql_escape") == 0)
            return LP_STRING;
          if (strcmp(func_name, "html_escape") == 0)
            return LP_STRING;
          if (strcmp(func_name, "strip_html") == 0)
            return LP_STRING;
          if (strcmp(func_name, "sanitize") == 0)
            return LP_STRING;
          /* Hash functions - return string */
          if (strcmp(func_name, "hash_md5") == 0)
            return LP_STRING;
          if (strcmp(func_name, "hash_sha256") == 0)
            return LP_STRING;
          /* Rate limit - return bool */
          if (strcmp(func_name, "check_rate_limit") == 0)
            return LP_BOOL;
          return LP_STRING;
        }
        if (imp->tier == MOD_TIER1_BASE64) {
          if (strcmp(func_name, "is_valid") == 0)
            return LP_BOOL;
          return LP_STRING;
        }
        if (imp->tier == MOD_TIER1_UNITTEST) {
          return LP_VOID;
        }
        if (imp->tier == MOD_TIER1_REGEX) {
          if (strcmp(func_name, "match") == 0 || strcmp(func_name, "search") == 0 ||
              strcmp(func_name, "replace") == 0 || strcmp(func_name, "replace_first") == 0 ||
              strcmp(func_name, "escape") == 0)
            return LP_STRING;
          if (strcmp(func_name, "is_match") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "count") == 0 || strcmp(func_name, "match_start") == 0 ||
              strcmp(func_name, "match_end") == 0)
            return LP_INT;
          /* find_all, split, match_groups return list */
          return LP_STRING; /* fallback */
        }
        if (imp->tier == MOD_TIER1_HASHLIB) {
          if (strcmp(func_name, "crc32") == 0)
            return LP_INT;
          return LP_STRING; /* md5, sha1, sha256, sha512, hmac, hash_file */
        }
        if (imp->tier == MOD_TIER1_CSV) {
          if (strcmp(func_name, "write") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "row_count") == 0)
            return LP_INT;
          if (strcmp(func_name, "to_string") == 0)
            return LP_STRING;
          if (strcmp(func_name, "set_delimiter") == 0)
            return LP_VOID;
          if (strcmp(func_name, "read") == 0 || strcmp(func_name, "parse") == 0 ||
              strcmp(func_name, "get_column") == 0 || strcmp(func_name, "parse_line") == 0 ||
              strcmp(func_name, "read_with_header") == 0)
            return LP_LIST;
          return LP_STRING; /* fallback */
        }
        if (imp->tier == MOD_TIER1_DATETIME) {
          if (strcmp(func_name, "now") == 0 || strcmp(func_name, "today") == 0 ||
              strcmp(func_name, "format") == 0)
            return LP_STRING;
          if (strcmp(func_name, "timestamp") == 0 || strcmp(func_name, "parse") == 0 ||
              strcmp(func_name, "diff") == 0 || strcmp(func_name, "add_seconds") == 0 ||
              strcmp(func_name, "add_days") == 0 || strcmp(func_name, "add_hours") == 0)
            return LP_FLOAT;
          if (strcmp(func_name, "year") == 0 || strcmp(func_name, "month") == 0 ||
              strcmp(func_name, "day") == 0 || strcmp(func_name, "weekday") == 0)
            return LP_INT;
          if (strcmp(func_name, "is_leap_year") == 0)
            return LP_BOOL;
          return LP_STRING; /* fallback */
        }
        if (imp->tier == MOD_TIER1_SOCKET) {
          if (strcmp(func_name, "create") == 0 || strcmp(func_name, "accept") == 0 ||
              strcmp(func_name, "send") == 0 || strcmp(func_name, "sendto") == 0)
            return LP_INT;
          if (strcmp(func_name, "connect") == 0 || strcmp(func_name, "bind") == 0 ||
              strcmp(func_name, "listen") == 0)
            return LP_BOOL;
          if (strcmp(func_name, "recv") == 0 || strcmp(func_name, "recvfrom") == 0 ||
              strcmp(func_name, "get_host_by_name") == 0 || strcmp(func_name, "get_local_ip") == 0)
            return LP_STRING;
          return LP_VOID; /* close, set_timeout, set_option */
        }
        if (imp->tier == MOD_TIER1_LOGGING) {
          if (strcmp(func_name, "get_level") == 0)
            return LP_STRING;
          return LP_VOID; /* debug, info, warning, error, critical, set_level, etc. */
        }
        if (imp->tier == MOD_TIER1_GUI) {
          if (strcmp(func_name, "window") == 0 || strcmp(func_name, "label") == 0 ||
              strcmp(func_name, "button") == 0 || strcmp(func_name, "input") == 0 ||
              strcmp(func_name, "textarea") == 0 || strcmp(func_name, "checkbox") == 0 ||
              strcmp(func_name, "radio") == 0 || strcmp(func_name, "dropdown") == 0 ||
        ...
        if (imp->tier == MOD_TIER1_DSA) {
          if (strcmp(func_name, "read_int") == 0)
            return LP_INT;
        if (strcmp(func_name, "read_float") == 0)
          return LP_FLOAT;
        if (strcmp(func_name, "read_str") == 0)
          return LP_STRING;
        if (strcmp(func_name, "read_line") == 0)
          return LP_STRING;
        /* Fast I/O - write functions */
        if (strncmp(func_name, "write", 5) == 0)
          return LP_VOID;
        if (strcmp(func_name, "flush") == 0)
          return LP_VOID;
        /* Number Theory */
        if (strcmp(func_name, "sieve") == 0)
          return LP_PTR; /* bool* */
        if (strcmp(func_name, "is_prime") == 0)
          return LP_BOOL;
        if (strcmp(func_name, "mod_pow") == 0)
          return LP_INT;
        if (strcmp(func_name, "mod_inverse") == 0)
          return LP_INT;
        if (strcmp(func_name, "extended_gcd") == 0)
          return LP_PTR; /* struct */
        if (strcmp(func_name, "prime_factors") == 0)
          return LP_PTR; /* struct */
        if (strcmp(func_name, "euler_phi") == 0)
          return LP_INT;
        if (strcmp(func_name, "count_divisors") == 0)
          return LP_INT;
        if (strcmp(func_name, "sum_divisors") == 0)
          return LP_INT;
        /* DSU */
        if (strcmp(func_name, "dsu_new") == 0)
          return LP_PTR;
        if (strcmp(func_name, "dsu_find") == 0)
          return LP_INT;
        if (strcmp(func_name, "dsu_union") == 0)
          return LP_BOOL;
        if (strcmp(func_name, "dsu_same") == 0)
          return LP_BOOL;
        if (strcmp(func_name, "dsu_size") == 0)
          return LP_INT;
        /* Heap */
        if (strcmp(func_name, "heap_new") == 0)
          return LP_PTR;
        if (strcmp(func_name, "heap_push") == 0)
          return LP_VOID;
        if (strcmp(func_name, "heap_pop") == 0)
          return LP_INT;
        if (strcmp(func_name, "heap_top") == 0)
          return LP_INT;
        if (strcmp(func_name, "heap_is_empty") == 0)
          return LP_BOOL;
        /* Fenwick Tree */
        if (strcmp(func_name, "fenwick_new") == 0)
          return LP_PTR;
        if (strcmp(func_name, "fenwick_add") == 0)
          return LP_VOID;
        if (strcmp(func_name, "fenwick_prefix_sum") == 0)
          return LP_INT;
        if (strcmp(func_name, "fenwick_range_sum") == 0)
          return LP_INT;
        return LP_INT;
      }
    ...
    if (node->call.func->type == NODE_ATTRIBUTE) {
      ...
      if (obj_type == LP_FILE) {
        ...
        if (strcmp(attr, "read") == 0)
          return LP_STRING;
        if (strcmp(attr, "write") == 0 || strcmp(attr, "close") == 0)
          return LP_VOID;
      }
      ...
      if (obj_type == LP_LIST) {
        ...
        if (strcmp(attr, "sort") == 0)
          return LP_VOID;
        if (strcmp(attr, "binary_search") == 0)
          return LP_INT;
      }
    }
  ...
  if (node->attribute.obj->type == NODE_NAME) {
    ...
    if (imp) {
      ...
      if (imp->tier == MOD_TIER1_OS)
        return LP_STRING;
      if (imp->tier == MOD_TIER1_SYS) {
        if (strcmp(node->attribute.attr, "platform") == 0)
          return LP_STRING;
        if (strcmp(node->attribute.attr, "maxsize") == 0)
          return LP_INT;
        return LP_STRING;
      }
...
case NODE_SUBSCRIPT: {
  ...
  if (obj_type == LP_NATIVE_ARRAY_FLOAT_1D ||
      obj_type == LP_NATIVE_ARRAY_FLOAT_2D)
    return LP_FLOAT;
  return LP_UNKNOWN;
}
case NODE_KWARG:
...
/* --- Code generation --- */
static void emit_cast(CodeGen *cg, Buffer *buf, AstNode *expr,
                      LpType target_type);
...
static void populate_function_symbol(CodeGen *cg, Symbol *sym, AstNode *node,
                                     const char *owner_class);
static int is_range_call(AstNode *node);
static void emit_native_int_array_access(CodeGen *cg, Buffer *buf,
                                         AstNode *obj_expr,
                                         AstNode *index_expr);
static void emit_native_numeric_minmax(CodeGen *cg, Buffer *buf, AstNode *node,
                                       int is_min, LpType result_type);
...
static void gen_thread_spawn_expr(CodeGen *cg, Buffer *buf, AstNode *node) {
  ...
  if (node->call.args.count < 1 || node->call.args.count > 4) {
    codegen_set_error(
        cg, "thread.spawn expects a worker function and up to 3 arguments");
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (!worker_node || worker_node->type != NODE_NAME) {
    codegen_set_error(cg,
                      "thread.spawn only supports named LP functions as "
                      "workers (lambdas not yet supported for thread safety)");
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (!worker_sym || !worker_sym->is_function) {
    codegen_set_error(cg,
                      "thread.spawn worker '%s' must be a named LP function",
                      worker_name);
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (worker_sym->is_variadic || worker_sym->has_kwargs) {
    codegen_set_error(cg,
                      "thread.spawn worker '%s' cannot use *args or **kwargs",
                      worker_name);
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (worker_sym->num_params > 3) {
    codegen_set_error(cg, "thread.spawn worker '%s' must accept 0-3 arguments",
                      worker_name);
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (worker_sym->type != LP_INT && worker_sym->type != LP_VOID &&
    ...
    codegen_set_error(
        cg, "thread.spawn worker '%s' must return int, void, float, or string",
        worker_name);
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (worker_sym->num_params == 0 && node->call.args.count != 1) {
    codegen_set_error(cg, "thread.spawn worker '%s' does not accept arguments",
                      worker_name);
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (worker_sym->num_params > 0 &&
      node->call.args.count != worker_sym->num_params + 1) {
    codegen_set_error(cg, "thread.spawn worker '%s' expects %d argument(s)",
                      worker_name, worker_sym->num_params);
    buf_write(buf, "NULL");
    return;
  }
  ...
  if (worker_sym->num_params >= 1) {
    if (worker_sym->first_param_type == LP_UNKNOWN) {
      codegen_set_error(
          cg, "thread.spawn worker '%s' has an unsupported argument type",
          worker_name);
      buf_write(buf, "NULL");
      return;
    }
    if (worker_sym->first_param_type == LP_OBJECT &&
        !worker_sym->first_param_class_name) {
      codegen_set_error(cg,
                        "thread.spawn worker '%s' has an object argument with "
                        "no class metadata",
                        worker_name);
      buf_write(buf, "NULL");
      return;
  ...
  snprintf(helper_name, sizeof(helper_name), "lp_thread_adapter_%d",
           adapter_id);
  ...
  param_c_type = lp_type_to_c_obj(worker_sym->first_param_type,
                                  worker_sym->first_param_class_name);
  ...
  buf_printf(buf, "%s *_lp_ctx = (%s*)malloc(sizeof(%s)); ", ctx_name, ctx_name,
             ctx_name);
  buf_write(buf, "if (_lp_ctx) { _lp_ctx->arg = ");
  gen_expr(cg, buf, node->call.args.items[1]);
  buf_printf(buf,
             "; _lp_thread = lp_thread_spawn(%s, _lp_ctx); if (!_lp_thread) "
             "free(_lp_ctx); } _lp_thread; })",
             helper_name);
}
static void emit_lp_val(CodeGen *cg, Buffer *buf, AstNode *expr) {
  ...
  switch (t) {
  case LP_INT:
    if (expr->type == NODE_CALL && expr->call.func->type == NODE_NAME &&
        strcmp(expr->call.func->name_expr.name, "int") == 0) {
...
static void gen_expr(CodeGen *cg, Buffer *buf, AstNode *node) {
  ...
  case NODE_FSTRING: {
    ...
    for (int i = 0; i < parts->count; i++) {
      ...
      if (part->type == NODE_STRING_LIT) {
        ...
        buf_printf(buf,
                   "_fs_len += snprintf(_fs_buf + _fs_len, sizeof(_fs_buf) - "
                   "_fs_len, \"%s\"); ",
                   part->str_lit.value);
      } else {
        ...
        if (part_type == LP_INT) {
          ...
          buf_write(buf, "_fs_len += snprintf(_fs_buf + _fs_len, "
                         "sizeof(_fs_buf) - _fs_len, \"%g\", ");
          gen_expr(cg, buf, part);
          buf_write(buf, "); ");
        } else if (part_type == LP_STRING) {
          buf_write(buf, "_fs_len += snprintf(_fs_buf + _fs_len, "
                         "sizeof(_fs_buf) - _fs_len, \"%s\", ");
          gen_expr(cg, buf, part);
          buf_write(buf, "); ");
  ...
  case NODE_LIST_EXPR: {
    ...
    /* All-integer literal list → inline LpIntArray* for zero-overhead table
     * access */
    if (list_expr_all_ints(node)) {
      buf_printf(buf, "({ LpIntArray* _la = lp_int_array_new(%d); ",
                 elems->count);
      for (int i = 0; i < elems->count; i++) {
        AstNode *e = elems->items[i];
        int64_t iv = 0;
  ...
  case NODE_TUPLE_EXPR: {
    buf_printf(buf, "({ LpTuple *_t = lp_tuple_new(%d); ",
               node->tuple_expr.elems.count);
    ...
    if (op == TOK_DSTAR) {
      ...
      if (lt == LP_VAL || rt == LP_VAL || lt == LP_DICT || rt == LP_DICT ||
        ...
        if (!rhs_is_pos_lit && node->bin_op.right->type == NODE_NAME) {
          Symbol *rs =
              scope_lookup(cg->scope, node->bin_op.right->name_expr.name);
        ...
        if (rhs_is_pos_lit) {
          ...
          if (node->bin_op.left->type == NODE_NAME) {
            Symbol *ls =
                scope_lookup(cg->scope, node->bin_op.left->name_expr.name);
            if (ls && ls->non_negative)
              lhs_nn = 1;
          }
          if (node->bin_op.right->type == NODE_NAME) {
            Symbol *rs2 =
                scope_lookup(cg->scope, node->bin_op.right->name_expr.name);
            if (rs2 && (rs2->non_negative || rs2->const_int_value > 0))
              rhs_nn = 1;
          }
          if (lhs_nn && rhs_nn) {
            buf_write(buf, "(");
      ...
      if (lt == LP_VAL || rt == LP_VAL || lt == LP_DICT || rt == LP_DICT ||
        ...
        if (rhs_is_pos_lit) {
          ...
          if (node->bin_op.left->type == NODE_NAME) {
            Symbol *ls2 =
                scope_lookup(cg->scope, node->bin_op.left->name_expr.name);
            if (ls2 && ls2->non_negative)
              lhs_nn2 = 1;
          }
          if (node->bin_op.right->type == NODE_NAME) {
            Symbol *rs3 =
                scope_lookup(cg->scope, node->bin_op.right->name_expr.name);
            if (rs3 && (rs3->non_negative || rs3->const_int_value > 0))
              rhs_nn2 = 1;
          }
          if (lhs_nn2 && rhs_nn2) {
            buf_write(buf, "(");
  ...
  case NODE_CALL: {
    ...
    if (node->call.func->type == NODE_ATTRIBUTE &&
      ...
      if (imp) {
        ...
        if (imp->tier == MOD_TIER1_HTTP) {
          ...
          if (strcmp(func_name, "get") != 0 && strcmp(func_name, "post") != 0 &&
              strcmp(func_name, "put") != 0 &&
              strcmp(func_name, "delete") != 0 &&
              strcmp(func_name, "patch") != 0) {
            codegen_set_error(cg,
                              "http.%s is not supported; available methods: "
        ...
        if (imp->tier == MOD_TIER1_JSON) {
          if (strcmp(func_name, "loads") != 0 &&
              strcmp(func_name, "dumps") != 0 &&
              strcmp(func_name, "parse") != 0) {
            codegen_set_error(cg,
                              "json.%s is not supported yet; only "
                              "json.loads/parse and json.dumps are available",
                              func_name);
            buf_write(buf, strcmp(func_name, "dumps") == 0 ? "\"\""
                                                           : "lp_val_null()");
            break;
          }
        ...
        /* ---- NEW MODULES ---- */
        if (imp->tier == MOD_TIER1_BASE64 || imp->tier == MOD_TIER1_REGEX ||
            imp->tier == MOD_TIER1_HASHLIB || imp->tier == MOD_TIER1_CSV ||
            imp->tier == MOD_TIER1_DATETIME || imp->tier == MOD_TIER1_SOCKET ||
            imp->tier == MOD_TIER1_LOGGING || imp->tier == MOD_TIER1_UNITTEST ||
            imp->tier == MOD_TIER1_GUI) {

          buf_printf(buf, "lp_%s_%s(", imp->module, func_name);
          for (int i = 0; i < node->call.args.count; i++) {
            if (i > 0)
              buf_write(buf, ", ");
            gen_expr(cg, buf, node->call.args.items[i]);
        ...
        } else if (strcmp(func_name, "memcpy_f32") == 0) {
          buf_write(buf, "lp_f32_memcpy(");
        } else if (strcmp(func_name, "memset_zero_f32") == 0) {
          buf_write(buf, "lp_f32_memset_zero(");
      ...
      if (imp->tier == MOD_TIER2_NUMPY) {
        ...
        if (strcmp(func_name, "array") == 0 && node->call.args.count == 1 &&
        ...
        if (strcmp(func_name, "sum") == 0 || strcmp(func_name, "mean") == 0 ||
            ...
            strcmp(func_name, "median") == 0 ||
            strcmp(func_name, "argmax") == 0 ||
        ...
        if (strcmp(func_name, "eye") == 0 ||
            strcmp(func_name, "identity") == 0) {
        ...
        if (strcmp(func_name, "reshape2d") == 0 ||
            strcmp(func_name, "reshape") == 0) {
      ...
      if (imp->tier == MOD_TIER3_PYTHON) {
        ...
        if (node->call.args.count == 0) {
          ...
          buf_printf(buf, "lp_py_args%d(",
                     node->call.args.count <= 3 ? node->call.args.count
          ...
          for (int i = 0; i < node->call.args.count; i++) {
            ...
            switch (at) {
            case LP_INT:
              buf_write(buf, "lp_py_from_int(");
              break;
            case LP_FLOAT:
              buf_write(buf, "lp_py_from_float(");
              break;
            case LP_STRING:
              buf_write(buf, "lp_py_from_str(");
              break;
            case LP_BOOL:
              buf_write(buf, "lp_py_from_bool(");
              break;
            default:
              buf_write(buf, "lp_py_from_int(");
              break;
        ...
        if (strcmp(attr, "join") == 0) {
          ...
          gen_expr(cg, buf,
                   node->call.func->attribute
                       .obj); /* The string itself is the first arg */
          for (int i = 0; i < node->call.args.count; i++) {
            buf_write(buf, ", ");
            gen_expr(cg, buf, node->call.args.items[i]);
          }
    ...
    if (node->call.func->type == NODE_NAME &&
      ...
      for (int i = 0; i < node->call.args.count; i++) {
        ...
        if (node->call.args.items[i]->type == NODE_CALL &&
            ...
            (strcmp(node->call.args.items[i]->call.func->name_expr.name,
                    "min") == 0 ||
             strcmp(node->call.args.items[i]->call.func->name_expr.name,
                    "max") == 0)) {
        ...
        switch (at) {
        case LP_FLOAT:
          buf_write(buf, "lp_print_float(");
          break;
        case LP_STRING:
          buf_write(buf, "lp_print_str(");
          break;
        case LP_BOOL:
          buf_write(buf, "lp_print_bool(");
          break;
        case LP_LIST:
          buf_write(buf, "lp_list_print(");
          break;
        case LP_ARRAY:
          buf_write(buf, "lp_np_print(");
          break;
        case LP_STR_ARRAY:
          buf_write(buf, "lp_print_str_array(");
          break;
        case LP_PYOBJ:
          buf_write(buf, "lp_py_print(");
          break;
        case LP_DICT:
          buf_write(buf, "lp_print_dict(");
          break;
        case LP_SET:
          buf_write(buf, "lp_print_set(");
          break;
        case LP_TUPLE:
          buf_write(buf, "lp_print_tuple(");
          break;
        case LP_VAL:
          buf_write(buf, "lp_print_val(");
          break;
        default:
          buf_write(buf, "lp_print_generic(");
          break;
        }
        gen_expr(cg, buf, node->call.args.items[i]);
        buf_write(buf, ")");
    ...
    if (node->call.func->type == NODE_NAME &&
      ...
      if (node->call.args.count == 1) {
        ...
        emit_lp_val(cg, buf, node->call.args.items[0]);
        buf_write(buf, "; ");
        buf_write(buf, "if (__lp_minmax_arg.type == LP_VAL_LIST && "
                       "__lp_minmax_arg.as.l->len > 0) { ");
        buf_write(buf, "__lp_minmax_res = __lp_minmax_arg.as.l->items[0]; ");
        buf_write(buf, "for (int64_t __lp_i = 1; __lp_i < "
                       "__lp_minmax_arg.as.l->len; __lp_i++) { ");
        buf_write(buf, "LpVal __v = __lp_minmax_arg.as.l->items[__lp_i]; ");
        if (is_min) {
          buf_write(
              buf,
              "if (lp_val_lt(__v, __lp_minmax_res)) __lp_minmax_res = __v; ");
        } else {
          buf_write(
              buf,
              "if (lp_val_gt(__v, __lp_minmax_res)) __lp_minmax_res = __v; ");
        }
        ...
        for (int i = 1; i < node->call.args.count; i++) {
          ...
          if (is_min) {
            ...
            buf_printf(buf, "%d, __lp_minmax_res)) __lp_minmax_res = __v_%d; ",
                       i, i);
          } else {
    ...
    if (node->call.func->type == NODE_NAME) {
      ...
      if (s && s->type == LP_CLASS) {
        ...
        buf_printf(
            buf,
            "({ LpObj_%s* _lp_tmp_obj = (LpObj_%s*)malloc(sizeof(LpObj_%s)); "
            "%s_init(_lp_tmp_obj); _lp_tmp_obj; })",
    ...
    if (node->call.func->type == NODE_ATTRIBUTE &&
        node->call.func->attribute.obj->type == NODE_NAME) {
      Symbol *s = scope_lookup(cg->scope,
                               node->call.func->attribute.obj->name_expr.name);
      if (s && s->type == LP_OBJECT && s->class_name) {
        const char *target_class = s->class_name;
        char method_key[256];
        snprintf(method_key, sizeof(method_key), "%s.%s", s->class_name,
                 node->call.func->attribute.attr);
        Symbol *ms = scope_lookup(cg->scope, method_key);
        if (ms && !can_access_member(cg, ms)) {
          codegen_set_error(cg, "Cannot access %s member '%s' of class '%s'",
                            ms->access == TOK_PRIVATE ? "private" : "protected",
                            node->call.func->attribute.attr,
                            ms->owner_class ? ms->owner_class : s->class_name);
        }
        if (ms && ms->owner_class)
          target_class = ms->owner_class;

        buf_printf(buf, "lp_%s_%s(", target_class,
                   node->call.func->attribute.attr);
        buf_printf(buf, "(LpObj_%s*)lp_%s", target_class,
                   s->name); /* Pass 'self' casted properly */
        for (int i = 0; i < node->call.args.count; i++) {
          buf_write(buf, ", ");
          gen_expr(cg, buf, node->call.args.items[i]);
        }
    ...
    for (int i = 0; i < node->call.args.count; i++) {
      if (node->call.args.items[i]->type == NODE_KWARG)
        has_kwarg_nodes = 1;
    }
    if (has_kwarg_nodes)
      has_kwargs_call = 1;
    ...
    if (has_unpack || has_kwargs_call) {
      ...
      if (has_unpack) {
        ...
        for (int i = fixed_params; i < node->call.args.count; i++) {
          ...
          if (node->call.is_unpacked_list && node->call.is_unpacked_list[i]) {
            ...
            buf_printf(buf, "lp_args_push(&_args, %d, (void*)(intptr_t)",
                       arg_type);
            gen_expr(cg, buf, node->call.args.items[i]);
            buf_write(buf, "); ");
          }
      ...
      if (has_kwargs_call) {
        ...
        for (int i = 0; i < node->call.args.count; i++) {
          ...
          if (arg->type == NODE_KWARG) {
            ...
            buf_write(buf, "); ");
          } else if (node->call.is_unpacked_dict &&
                     node->call.is_unpacked_dict[i]) {
      ...
      /* Cleanup */
      if (has_unpack)
        buf_write(buf, "lp_args_free(&_args); ");
      buf_write(buf, "_res; })");
    } else {
      if (node->call.func->type == NODE_NAME) {
        buf_printf(buf, "lp_%s(", node->call.func->name_expr.name);
...
case NODE_LIST_COMP: {
  ...
  /* Define loop var in a new scope so it can be resolved during expression
   * generation */
  ...
  if (is_range_call(node->list_comp.iter)) {
    ...
    if (args->count == 1) {
      buf_printf(buf, "for (int64_t lp_%s = 0; lp_%s < ", node->list_comp.var,
                 node->list_comp.var);
      ...
      if (args->count >= 3) {
        ...
        buf_write(buf, "; 0;) { break; } "); /* Hack đóng lệnh for rỗng nêú cần,
                                                hoặc viết lại block */
        ...
        buf_printf(buf, "; lp_%s += __step_%s) { ", node->list_comp.var,
                   node->list_comp.var);
      } else {
        buf_printf(buf, "; lp_%s < ", node->list_comp.var);
        gen_expr(cg, buf, args->items[1]);
    ...
    if (iter_type == LP_NATIVE_ARRAY_1D) {
      ...
      buf_write(buf, "; ");
      buf_printf(buf, "int64_t __lp_lc_len_%d = __lp_lc_iter_%d->len; ",
                 cur_loop, cur_loop);
      buf_printf(buf,
                 "for (int64_t __lp_lc_i_%d = 0; __lp_lc_i_%d < "
                 "__lp_lc_len_%d; __lp_lc_i_%d++) { ",
      ...
      gen_expr(cg, buf, node->list_comp.iter);
      buf_write(buf, "; ");
      buf_printf(buf, "int64_t __lp_lc_len_%d = __lp_lc_arr_%d.len; ", cur_loop,
                 cur_loop);
      buf_printf(buf,
                 "for (int64_t __lp_lc_i_%d = 0; __lp_lc_i_%d < "
                 "__lp_lc_len_%d; __lp_lc_i_%d++) { ",
      ...
      gen_expr(cg, buf, node->list_comp.iter);
      buf_write(buf, "; ");
      buf_printf(buf, "int64_t __lp_lc_len_%d = __lp_lc_iter_%d.count; ",
                 cur_loop, cur_loop);
      buf_printf(buf,
                 "for (int64_t __lp_lc_i_%d = 0; __lp_lc_i_%d < "
                 "__lp_lc_len_%d; __lp_lc_i_%d++) { ",
                 cur_loop, cur_loop, cur_loop, cur_loop);
      buf_printf(buf,
                 "const char *lp_%s = __lp_lc_iter_%d.items[__lp_lc_i_%d]; ",
                 node->list_comp.var, cur_loop, cur_loop);
    } else if (iter_type == LP_VAL || iter_type == LP_LIST ||
               iter_type == LP_PYOBJ || iter_type == LP_ARRAY ||
               iter_type == LP_DICT || iter_type == LP_SET ||
               iter_type == LP_TUPLE) {
      ...
      buf_write(buf, "; ");
      buf_printf(buf, "int64_t __lp_lc_len_%d = lp_val_len(__lp_lc_iter_%d); ",
                 cur_loop, cur_loop);
      buf_printf(buf,
                 "for (int64_t __lp_lc_i_%d = 0; __lp_lc_i_%d < "
                 "__lp_lc_len_%d; __lp_lc_i_%d++) { ",
...
case NODE_LAMBDA: {
  ...
  for (int i = 0; i < node->lambda_expr.params.count; i++) {
    ...
    LpType pt =
        lp->type_ann ? type_from_annotation(cg, lp->type_ann) : LP_FLOAT;
...
case NODE_SUBSCRIPT: {
  ...
  if (obj_type == LP_DICT) {
  ...
  } else if (obj_type == LP_PYOBJ || obj_type == LP_VAL ||
             obj_type == LP_LIST) {
    LpType key_type = infer_type(cg, node->subscript.index);
    if (key_type == LP_STRING) {
    ...
    emit_native_int_array_access(cg, buf, node->subscript.obj,
                                 node->subscript.index);
  } else if (obj_type == LP_NATIVE_ARRAY_F32_1D) {
  ...
  } else if (obj_type == LP_NATIVE_ARRAY_I8_1D) {
...
case NODE_ATTRIBUTE: {
  ...
  if (node->attribute.obj->type == NODE_NAME) {
    ...
    if (s && s->type == LP_OBJECT) {
      if (s->class_name) {
        char field_key[256];
        snprintf(field_key, sizeof(field_key), "%s.%s", s->class_name,
                 node->attribute.attr);
        Symbol *mem = scope_lookup(cg->scope, field_key);
        if (mem && !can_access_member(cg, mem)) {
          codegen_set_error(
              cg, "Cannot access %s member '%s' of class '%s'",
  ...
  if (node->attribute.obj->type == NODE_NAME) {
    ...
    if (imp) {
      ...
      if (imp->tier == MOD_TIER3_PYTHON) {
        buf_printf(buf, "lp_py_getattr(lp_pymod_%s, \"%s\")", imp->alias,
                   node->attribute.attr);
        break;
      }
...
case NODE_DICT_COMP: {
  ...
  if (is_range_call(node->dict_comp.iter)) {
    ...
    if (args->count == 1) {
      buf_printf(buf, "for (int64_t lp_%s = 0; lp_%s < ", node->dict_comp.var,
                 node->dict_comp.var);
      gen_expr(cg, buf, args->items[0]);
      ...
      if (args->count >= 3) {
        ...
        buf_write(buf, " < ");
        gen_expr(cg, buf, args->items[1]);
        buf_printf(buf, "; lp_%s += __step_%s) { ", node->dict_comp.var,
                   node->dict_comp.var);
      } else {
        buf_printf(buf, "; lp_%s++) { ", node->dict_comp.var);
      }
    ...
    buf_printf(buf, ".len; _lp_i++) { int64_t lp_%s = (int64_t)",
               node->dict_comp.var);
    gen_expr(cg, buf, node->dict_comp.iter);
...
case NODE_AWAIT_EXPR:
   ...
   * For true async, a coroutine-based or callback implementation would be
   * needed.
   */
  buf_write(buf, "/* await */ ");
  if (node->await_expr.expr) {
...
static void emit_native_int_array_init_expr(CodeGen *cg, Buffer *buf,
                                            AstNode *elem_expr,
                                            AstNode *count_expr) {
  int64_t literal = 0;
...
static void emit_native_int_array_access(CodeGen *cg, Buffer *buf,
                                         AstNode *obj_expr,
                                         AstNode *index_expr) {
  /* If obj is a simple name and we have a _raw_ unwrap, use it directly:
   * _raw_arr[i] */
...
static void emit_native_numeric_minmax(CodeGen *cg, Buffer *buf, AstNode *node,
  ...
  for (int i = 1; i < node->call.args.count; i++) {
    ...
    buf_write(buf, "); ");
    buf_printf(buf,
               "if (__lp_minmax_arg_%d %s __lp_minmax_res) __lp_minmax_res = "
               "__lp_minmax_arg_%d; ",
               i, cmp, i);
  }
...
static void emit_cast(CodeGen *cg, Buffer *buf, AstNode *expr,
  ...
  if (target_type == LP_VAL) {
    ...
    gen_expr(cg, buf, expr);
    buf_write(buf, ")");
    /* FIX: LpIntArray*/ /* or LpI8Array* */ /* passed to a function that
                                                expects LpList* */
  } else if (target_type == LP_LIST && (expr_type == LP_NATIVE_ARRAY_1D ||
                                        expr_type == LP_NATIVE_ARRAY_2D)) {
    buf_write(buf, "lp_int_array_to_list(");
    gen_expr(cg, buf, expr);
    buf_write(buf, ")");
  } else if (target_type == LP_LIST && expr_type == LP_NATIVE_ARRAY_I8_1D) {
    buf_write(buf, "lp_i8_array_to_list(");
    gen_expr(cg, buf, expr);
    buf_write(buf, ")");
  } else if (target_type == LP_LIST && expr_type == LP_NATIVE_ARRAY_I32_1D) {
    /* LpI32Array* -> LpList* on-the-fly conversion */
    buf_write(buf, "({ LpI32Array* _t32 = ");
    gen_expr(cg, buf, expr);
    buf_write(buf, "; LpList *_l32 = lp_list_new();");
    buf_write(buf, " for(int64_t _ii=0;_ii<_t32->len;_ii++)");
    buf_write(buf, "  lp_list_append(_l32,lp_val_int(_t32->data[_ii]));");
    buf_write(buf, " _l32; })");
  } else if (target_type == LP_LIST && expr_type == LP_NATIVE_ARRAY_FLOAT_1D) {
    buf_write(buf, "({ LpFloatArray* _tf = ");
    gen_expr(cg, buf, expr);
    buf_write(buf, "; LpList *_lf = lp_list_new();");
    buf_write(buf, " for(int64_t _ii=0;_ii<_tf->len;_ii++)");
    buf_write(buf, "  lp_list_append(_lf,lp_val_float(_tf->data[_ii]));");
    buf_write(buf, " _lf; })");
  } else {
    gen_expr(cg, buf, expr);
  }
...
static void gen_stmt(CodeGen *cg, Buffer *buf, AstNode *node, int indent) {
  ...
  case NODE_ASSIGN: {
    ...
    /* If annotated as f32[], keep that for [0.0]*N pattern */
    if (t == LP_NATIVE_ARRAY_F32_1D && node->assign.value)
    ...
    if (t == LP_NATIVE_ARRAY_I32_1D && node->assign.value)
      match_native_int_repeat_expr(cg, node->assign.value, &auto_array_elem,
                                   &auto_array_count);
    /* If annotated as i8[], match [val]*N pattern */
    ...
    /* Auto-promote list annotation to native int array for small all-int
     * literals e.g. dx: list = [0, 0, 1, -1]  →  LpIntArray*  (avoids LpVal
     * boxing) */
    ...
    if (cg->scope->parent == NULL && node->assign.value &&
        node->assign.value->type == NODE_LAMBDA) {
    ...
    if (dot) {
      *dot = '\0'; /* split into obj and attr */
      ...
      if (obj_sym && obj_sym->type == LP_OBJECT && obj_sym->class_name) {
        char field_key[256];
        Symbol *mem;
        snprintf(field_key, sizeof(field_key), "%s.%s", obj_sym->class_name,
                 attr_name);
        ...
        if (mem) {
          ...
          if (!can_access_member(cg, mem)) {
            codegen_set_error(
                cg, "Cannot access %s member '%s' of class '%s'",
                mem->access == TOK_PRIVATE ? "private" : "protected", attr_name,
      ...
      /* SỬA TẠI ĐÂY: Dùng emit_cast để tự động bọc bằng hàm ép kiểu C (như
       * lp_int) */
      ...
      if (!existing) {
        ...
        if (t == LP_OBJECT && node->assign.value->type == NODE_CALL &&
          ...
          Symbol *cs = scope_lookup(
              cg->scope, node->assign.value->call.func->name_expr.name);
          if (cs && cs->type == LP_CLASS)
            class_name = cs->name;
        } else if (t == LP_OBJECT && node->assign.value->type == NODE_NAME) {
          Symbol *os =
              scope_lookup(cg->scope, node->assign.value->name_expr.name);
          if (os)
            class_name = os->class_name;
        } else if (t == LP_OBJECT && node->assign.value->type == NODE_CALL &&
                   node->assign.value->call.func->type == NODE_ATTRIBUTE &&
                   node->assign.value->call.func->attribute.obj->type ==
                       NODE_NAME &&
                   strcmp(node->assign.value->call.func->attribute.obj
                              ->name_expr.name,
                          "memory") == 0 &&
                   strcmp(node->assign.value->call.func->attribute.attr,
                          "cast") == 0 &&
          ...
          if (_ns) {
            ...
            if (_v && _v->type == NODE_BIN_OP) {
              ...
              if (!_Rpos && _bR->type == NODE_NAME) {
                ...
                if (_bRS && _bRS->const_int_value > 0)
                  _Rpos = 1;
              }
              ...
              if (_v->bin_op.op == TOK_PLUS) {
                ...
                if (_bL->type == NODE_NAME) {
                  ...
                  if (_bLS && _bLS->non_negative)
                    _Lnn = 1;
                }
                int64_t _bLv = 0;
                if (get_int_literal_value(_bL, &_bLv) && _bLv >= 0)
                  _Lnn = 1;
                if (_Lnn && _Rpos)
                  _ns->non_negative = 1;
              }
        ...
        if (existing)
          existing->declared = 1; /* Mark as declared */

        /* Handle native array allocation */
        if (t == LP_NATIVE_ARRAY_1D || t == LP_NATIVE_ARRAY_2D) {
          write_indent(assign_buf, assign_indent);
          if (node->assign.type_ann &&
              is_native_array_type(node->assign.type_ann)) {
            char *sizes[4] = {NULL, NULL, NULL, NULL};
            ...
            if (dims == 1) {
              ...
              buf_printf(assign_buf,
                         "LpIntArray2D* lp_%s = lp_int_array2d_new(%s, %s);",
                         node->assign.name, c_size0 ? c_size0 : "1",
                         c_size1 ? c_size1 : "1");
              if (c_size0)
                free(c_size0);
              if (c_size1)
          ...
          } else if (t == LP_NATIVE_ARRAY_1D && auto_array_elem &&
                     auto_array_count) {
          ...
          buf_write(assign_buf, "\n");
                   ...
                   t == LP_NATIVE_ARRAY_FLOAT_2D) {
          /* Float array: float[n] annotation or [0.0]*N pattern */
          write_indent(assign_buf, assign_indent);
          if (node->assign.type_ann &&
              is_float_array_annotation(node->assign.type_ann)) {
            char *sizes[4] = {NULL, NULL, NULL, NULL};
            int dims = parse_native_array_dims(node->assign.type_ann, sizes);
            if (dims == 1) {
              char *c_size = convert_size_expr_to_c(sizes[0]);
              ...
              buf_printf(
                  assign_buf,
                  "LpFloatArray2D* lp_%s = lp_float_array2d_new(%s, %s);",
                  node->assign.name, c_size0 ? c_size0 : "1",
                  c_size1 ? c_size1 : "1");
              if (c_size0)
                free(c_size0);
          ...
          } else if (t == LP_NATIVE_ARRAY_FLOAT_1D && auto_array_elem &&
                     auto_array_count) {
            int is_zero = (auto_array_elem->type == NODE_FLOAT_LIT &&
                           auto_array_elem->float_lit.value == 0.0);
            if (is_zero) {
              buf_printf(assign_buf,
                         "LpFloatArray* lp_%s = lp_float_array_new(",
                         node->assign.name);
              buf_printf(assign_buf, "/* _raw_upd_marker_%s */",
                         node->assign.name);
              gen_expr(cg, assign_buf, auto_array_count);
              buf_write(assign_buf, ");");
            } else {
              buf_printf(assign_buf,
                         "LpFloatArray* lp_%s = lp_float_array_repeat(",
                         node->assign.name);
              buf_printf(assign_buf, "/* _raw_upd_marker_%s */",
                         node->assign.name);
          ...
          write_indent(assign_buf, assign_indent);
          if (t == LP_NATIVE_ARRAY_FLOAT_1D && auto_array_elem &&
              auto_array_count) {
            /* [val] * count form */
            double fval = 0.0;
            int is_zero = (auto_array_elem->type == NODE_FLOAT_LIT &&
            ...
            buf_printf(assign_buf, "LpFloatArray2D* lp_%s = NULL;",
                       node->assign.name);
          } else {
            buf_printf(assign_buf, "LpFloatArray* lp_%s = NULL;",
                       node->assign.name);
          ...
          buf_write(assign_buf, "\n");
        } else if (t == LP_NATIVE_ARRAY_F32_1D) {
          /* f32[] — float32 array: 2x memory bandwidth vs double */
          write_indent(assign_buf, assign_indent);
          int emitted_f32 = 0;
          if (node->assign.type_ann &&
              strncmp(node->assign.type_ann, "f32[", 4) == 0) {
            char *sizes[4] = {NULL, NULL, NULL, NULL};
            int dims = parse_native_array_dims(node->assign.type_ann, sizes);
            if (dims >= 1) {
              char *c_size = convert_size_expr_to_c(sizes[0]);
              buf_printf(assign_buf,
                         "LpF32Array* lp_%s = lp_f32_array_new(%s);",
                         node->assign.name, c_size ? c_size : "1");
              if (c_size) {
                free(c_size);
              }
          ...
          if (!emitted_f32 && auto_array_count) {
            buf_printf(assign_buf, "LpF32Array* lp_%s = lp_f32_array_new(",
                       node->assign.name);
            gen_expr(cg, assign_buf, auto_array_count);
            buf_write(assign_buf, ");");
            emitted_f32 = 1;
          ...
          /* i32[] / i32[][] — int32_t arrays: 2x denser than int64, better
           * cache */
          write_indent(assign_buf, assign_indent);
          ...
          if (node->assign.type_ann) {
            ...
            if (dims >= 1) {
              char *c_size = convert_size_expr_to_c(sizes[0]);
              buf_printf(assign_buf,
                         "LpI32Array* lp_%s = lp_i32_array_new(%s);",
                         node->assign.name, c_size ? c_size : "1");
              if (c_size)
                free(c_size);
              emitted_i32 = 1;
          ...
          if (!emitted_i32 && auto_array_count) {
            buf_printf(assign_buf, "LpI32Array* lp_%s = lp_i32_array_new(",
                       node->assign.name);
            gen_expr(cg, assign_buf, auto_array_count);
            buf_write(assign_buf, ");");
            emitted_i32 = 1;
          ...
          if (!emitted_i32)
            buf_printf(assign_buf, "LpI32Array* lp_%s = NULL;",
                       node->assign.name);
          buf_write(assign_buf, "\n");
        } else if (t == LP_NATIVE_ARRAY_I8_1D) {
          /* i8[] — int8_t byte arrays: 8x denser than int64, ideal for
           * sieve/visited */
          write_indent(assign_buf, assign_indent);
          int emitted_i8 = 0;
          if (node->assign.type_ann &&
              strncmp(node->assign.type_ann, "i8[", 3) == 0) {
            char *sizes[4] = {NULL, NULL, NULL, NULL};
            int dims = parse_native_array_dims(node->assign.type_ann, sizes);
            if (dims >= 1) {
              char *c_size = convert_size_expr_to_c(sizes[0]);
              buf_printf(assign_buf, "LpI8Array* lp_%s = lp_i8_array_new(%s);",
                         node->assign.name, c_size ? c_size : "1");
              if (c_size)
                free(c_size);
              emitted_i8 = 1;
          ...
          /* unsized i8[] + [val]*N pattern */
          if (!emitted_i8 && auto_array_count) {
            if (auto_array_elem && auto_array_elem->type == NODE_INT_LIT) {
              int64_t iv = auto_array_elem->int_lit.value;
              buf_printf(assign_buf,
                         "LpI8Array* lp_%s = lp_i8_array_repeat((int8_t)%lld, ",
                         node->assign.name, (long long)iv);
            } else {
              buf_printf(assign_buf,
                         "LpI8Array* lp_%s = lp_i8_array_repeat(0, ",
                         node->assign.name);
            }
            gen_expr(cg, assign_buf, auto_array_count);
            buf_write(assign_buf, ");");
          ...
          if (!emitted_i8)
            buf_printf(assign_buf, "LpI8Array* lp_%s = NULL;",
                       node->assign.name);
          /* define raw pointer macro for fast access */
          buf_printf(assign_buf, " /* i8 raw */ ");
          buf_write(assign_buf, "\n");
          /* inject _raw_ macro for this array */
          buf_printf(
              assign_buf,
              "#define _raw_%s ((int8_t* "
              "__restrict__)__builtin_assume_aligned(lp_%s->data, 32))\n",
              node->assign.name, node->assign.name);
        } else if (assign_buf == &cg->helpers && node->assign.value &&
                   node->assign.value->type == NODE_LAMBDA) {
          ...
          buf_write(assign_buf,
                    lambda_expr_buf.data ? lambda_expr_buf.data : "");
          buf_write(assign_buf, ";\n");
          ...
          if (node->assign.value && node->assign.value->type == NODE_LAMBDA) {
            ...
            buf_printf(assign_buf, "%s lp_%s", lp_type_to_c_obj(t, class_name),
                       node->assign.name);
          }
        ...
        if (!existing->declared) {
          ...
          /* Handle native array allocation */
          if (existing->type == LP_NATIVE_ARRAY_1D ||
              existing->type == LP_NATIVE_ARRAY_2D) {
            write_indent(assign_buf, assign_indent);
            if (node->assign.type_ann &&
                is_native_array_type(node->assign.type_ann)) {
              char *sizes[4] = {NULL, NULL, NULL, NULL};
              ...
              if (dims == 1) {
                ...
                /* 2D array: LpIntArray2D* lp_arr = lp_int_array2d_new(rows,
                 * cols) */
                char *c_size0 = convert_size_expr_to_c(sizes[0]);
                char *c_size1 = convert_size_expr_to_c(sizes[1]);
                buf_printf(assign_buf,
              ...
              buf_printf(assign_buf,
                         "LpIntArray* lp_%s = lp_int_array_new(%d);\n",
                         node->assign.name, elems->count);
              for (int _ei = 0; _ei < elems->count; _ei++) {
                AstNode *_e = elems->items[_ei];
                int64_t _iv = 0;
                if (_e->type == NODE_INT_LIT)
                  _iv = _e->int_lit.value;
                else if (_e->type == NODE_UNARY_OP &&
                         _e->unary_op.op == TOK_MINUS &&
          ...
          } else if (existing->type == LP_NATIVE_ARRAY_FLOAT_1D ||
                     existing->type == LP_NATIVE_ARRAY_FLOAT_2D) {
            /* Float native array first use */
            write_indent(assign_buf, assign_indent);
            if (existing->type == LP_NATIVE_ARRAY_FLOAT_1D && auto_array_elem &&
                auto_array_count) {
              int is_zero = (auto_array_elem->type == NODE_FLOAT_LIT &&
                             auto_array_elem->float_lit.value == 0.0);
              if (is_zero) {
          ...
          } else if (existing->type == LP_NATIVE_ARRAY_F32_1D) {
            /* f32[] first assignment */
            write_indent(assign_buf, assign_indent);
            if (auto_array_count) {
              buf_printf(assign_buf, "LpF32Array* lp_%s = lp_f32_array_new(",
                         node->assign.name);
          ...
          } else if (existing->type == LP_NATIVE_ARRAY_I32_1D ||
                     existing->type == LP_NATIVE_ARRAY_I32_2D) {
            /* i32[] first assignment — use repeat for non-zero init, new for
             * zero */
            write_indent(assign_buf, assign_indent);
            if (auto_array_count && auto_array_elem) {
              int64_t iv =
                  (auto_array_elem->type == NODE_INT_LIT)
                      ? auto_array_elem->int_lit.value
                  : (auto_array_elem->type == NODE_UNARY_OP &&
                     ...
                     auto_array_elem->unary_op.operand->type == NODE_INT_LIT)
                      ? -auto_array_elem->unary_op.operand->int_lit.value
                      : 0;
              if (iv == 0) {
                buf_printf(assign_buf, "LpI32Array* lp_%s = lp_i32_array_new(",
                           node->assign.name);
                ...
                buf_printf(
                    assign_buf,
                    "LpI32Array* lp_%s = lp_i32_array_repeat((int32_t)%lld, ",
                    node->assign.name, (long long)iv);
                gen_expr(cg, assign_buf, auto_array_count);
                buf_write(assign_buf, ");\n");
              }
          ...
          } else if (existing->type == LP_NATIVE_ARRAY_I8_1D) {
            /* i8[] first assignment — use lp_i8_array_repeat for [val]*N
             * pattern */
            write_indent(assign_buf, assign_indent);
            if (auto_array_count && auto_array_elem) {
              int64_t iv = (auto_array_elem->type == NODE_INT_LIT)
                               ? auto_array_elem->int_lit.value
                               : 0;
              buf_printf(assign_buf,
                         "LpI8Array* lp_%s = lp_i8_array_repeat((int8_t)%lld, ",
              ...
              buf_printf(assign_buf, "LpI8Array* lp_%s = lp_i8_array_new(",
                         node->assign.name);
              gen_expr(cg, assign_buf, auto_array_count);
              buf_write(assign_buf, ");\n");
            } else {
             ...
             */
            if (existing->type == LP_NATIVE_ARRAY_1D && auto_array_elem &&
                auto_array_count) {
              ...
              buf_printf(assign_buf, "%s lp_%s",
                         lp_type_to_c_obj(existing->type, class_name),
                         node->assign.name);
              if (node->assign.value) {
                buf_write(assign_buf, " = ");
                emit_cast(cg, assign_buf, node->assign.value, existing->type);
          ...
          if (assign_buf == &cg->helpers && node->assign.value &&
              node->assign.value->type == NODE_LAMBDA) {
            Buffer lambda_expr_buf;
            buf_init(&lambda_expr_buf);
            gen_expr(cg, &lambda_expr_buf, node->assign.value);
  ...
  case NODE_SUBSCRIPT_ASSIGN: {
    ...
    if (obj_type == LP_NATIVE_ARRAY_1D) {
      ...
      if (node->subscript_assign.op != TOK_ASSIGN) {
        ...
        emit_native_int_array_access(cg, buf, node->subscript_assign.obj,
                                     node->subscript_assign.index);
        buf_write(buf, " ");
      ...
      if (node->subscript_assign.op != TOK_ASSIGN) {
        ...
        buf_printf(buf, "]) %s (int8_t)(", op_str);
      } else {
        buf_write(buf, "]) = (int8_t)(");
      }
      gen_expr(cg, buf, node->subscript_assign.value);
      buf_write(buf, ");\n");
    } else if (obj_type == LP_NATIVE_ARRAY_2D) {
      ...
      /* Handle augmented assignment to a subscript (like arr[0] += 1) vs direct
       * assignment (arr[0] = 1) */
      ...
      /* Fast path: dict[str_key] = int_val  →  lp_dict_set(d, key,
       * lp_val_int(v)) Bypasses lp_val_set_item boxing overhead entirely. */
        ...
        if (then_s->type == NODE_SUBSCRIPT_ASSIGN &&
          ...
          if (else_s->type == NODE_SUBSCRIPT_ASSIGN &&
            ...
            if (then_obj->type == NODE_NAME && else_obj->type == NODE_NAME &&
                strcmp(then_obj->name_expr.name, else_obj->name_expr.name) ==
                    0 &&
                then_idx->type == NODE_NAME && else_idx->type == NODE_NAME &&
                strcmp(then_idx->name_expr.name, else_idx->name_expr.name) ==
                    0) {

              write_indent(buf, indent);
              buf_printf(buf, "lp_dict_inc_int(lp_%s, lp_%s, %lldLL);\n",
                         then_obj->name_expr.name, then_idx->name_expr.name,
                         (long long)delta);
              emitted_inc = 1;
            }
          }
  ...
  case NODE_FOR: {
    ...
    if (is_range_call(node->for_stmt.iter)) {
      ...
      if (body_peek->count == 1) {
        ...
        if (stmt_peek->type == NODE_SUBSCRIPT_ASSIGN &&
          ...
          if (lhs && lidx && rhs && lidx->type == NODE_NAME &&
              strcmp(lidx->name_expr.name, var_peek) == 0) {
            /* Check dst is native int array */
            LpType lhs_t = (lhs->type == NODE_NAME) ? ({
              Symbol *s = scope_lookup(cg->scope, lhs->name_expr.name);
              s ? s->type : LP_UNKNOWN;
            })
                                                    : LP_UNKNOWN;
            /* RHS: subscript of native int array with same loop var */
            if ((lhs_t == LP_NATIVE_ARRAY_1D) && rhs->type == NODE_SUBSCRIPT &&
                rhs->subscript.index->type == NODE_NAME &&
                strcmp(rhs->subscript.index->name_expr.name, var_peek) == 0) {
              Symbol *_rhs_sym =
                  (rhs->subscript.obj->type == NODE_NAME)
                      ? scope_lookup(cg->scope,
                                     rhs->subscript.obj->name_expr.name)
                      : NULL;
              LpType rhs_t = _rhs_sym ? _rhs_sym->type : LP_UNKNOWN;
              if ((rhs_t == LP_NATIVE_ARRAY_1D ||
                   rhs_t == LP_NATIVE_ARRAY_I32_1D) &&
                  lhs_t == rhs_t && args_peek->count == 1) {
                const char *esz2 =
                    (lhs_t == LP_NATIVE_ARRAY_I32_1D) ? "int32_t" : "int64_t";
                write_indent(buf, indent);
    ...
    if (is_range_call(node->for_stmt.iter)) {
      ...
      /* Fix 2a: ivdep for pure array-write loops (no loop-carried deps) */
      {
        ...
        for (int _bi = 0; _bi < node->for_stmt.body.count && all_array_writes;
             _bi++) {
       ...
       * With -ffast-math + AVX-512, GCC uses 8-wide SIMD reduction → 4-8x
       * speedup. This safely handles: sum, product, min/max accumulation loops.
       */
      {
        int is_reduction = 0;
        ...
        if (node->for_stmt.body.count == 1 + _body_start) {
          ...
          /* Case 2: assign  acc = acc + expr  (LP generates this form, float
           * only) Handles: total = total+x, total = (total+a)+b (nested +) */
          if (_s->type == NODE_ASSIGN && _s->assign.value &&
            ...
            while (
                _leaf->type == NODE_BIN_OP &&
                (_leaf->bin_op.op == TOK_PLUS || _leaf->bin_op.op == TOK_STAR))
        ...
        if (is_reduction && red_var && red_op) {
          write_indent(buf, indent);
          /* simdlen(8): request 8-wide AVX-512 (8 doubles = 512-bit zmm
           * registers) Combined with -lmvec and avx512f target, enables
           * vectorized sin/cos. */
       ...
       */
      {
        ...
        if (args->count == 1) {
          if (get_int_literal_value(args->items[0], &_b) && _b > 0 &&
              _b <= 1073741824LL)
            _use_int = 1;
          else if (args->items[0]->type == NODE_NAME) {
            Symbol *_bs =
                scope_lookup(cg->scope, args->items[0]->name_expr.name);
            if (_bs && _bs->const_int_value > 0 &&
                _bs->const_int_value <= 1073741824LL)
              _use_int = 1;
          }
        }
        /* Safety check: recursively scan for lp_i * large_literal anywhere in
         * body. This catches: a[i]=(i*1103515245+12345)%26 where the multiply
         * is nested. */
        if (_use_int) {
          /* Recursive expression scanner */
          int _has_overflow = 0;
/* Inline recursive check via stack */
#define _CHECK_EXPR_OVERFLOW(e_)                                               \
  do {                                                                         \
    AstNode *_stk[32];                                                         \
    int _sp = 0;                                                               \
    if (e_)                                                                    \
    ...
    while (_sp > 0 && !_has_overflow) {                                        \
      ...
      if (_e->type == NODE_BIN_OP && _e->bin_op.op == TOK_STAR) {              \
        ...
        int64_t _mv = 0;                                                       \
        if ((_L->type == NODE_NAME &&                                          \
             strcmp(_L->name_expr.name, _lvar) == 0 &&                         \
             get_int_literal_value(_R, &_mv) && _mv > 65535) ||                \
            (_R->type == NODE_NAME &&                                          \
             strcmp(_R->name_expr.name, _lvar) == 0 &&                         \
             get_int_literal_value(_L, &_mv) && _mv > 65535))                  \
          _has_overflow = 1;                                                   \
      }                                                                        \
      if (_e->type == NODE_BIN_OP) {                                           \
        if (_sp < 30) {                                                        \
          _stk[_sp++] = _e->bin_op.left;                                       \
        }                                                                      \
        if (_sp < 30) {                                                        \
          _stk[_sp++] = _e->bin_op.right;                                      \
        }                                                                      \
      }                                                                        \
      if (_e->type == NODE_UNARY_OP && _sp < 30)                               \
        _stk[_sp++] = _e->unary_op.operand;                                    \
    }                                                                          \
  } while (0)
          for (int _bi = 0; _bi < node->for_stmt.body.count && _use_int;
               _bi++) {
            AstNode *_s = node->for_stmt.body.items[_bi];
...
#undef _CHECK_EXPR_OVERFLOW
        ...
        if (args->count == 1) {
          if (_use_int) {
            buf_printf(buf, "for (int lp_%s = 0; lp_%s < ", node->for_stmt.var,
                       node->for_stmt.var);
          } else {
          ...
          /* Safe int counter for range(start, end) when both bounds are safe
           * literals */
          {
            int64_t _s2 = 0, _e2 = 0;
            int _use_int2 = 0;
            ...
            for (int _bi2 = 0;
                 _bi2 < node->for_stmt.body.count && !_has_overflow2; _bi2++) {
  ...
  do {                                                                         \
    ...
    while (_sp > 0 && !_has_overflow2) {                                       \
      ...
      if (_e->type == NODE_BIN_OP && _e->bin_op.op == TOK_STAR) {              \
        ...
        int64_t _mv = 0;                                                       \
        if ((_L->type == NODE_NAME &&                                          \
             strcmp(_L->name_expr.name, node->for_stmt.var) == 0 &&            \
             get_int_literal_value(_R, &_mv) && _mv > 65535) ||                \
            (_R->type == NODE_NAME &&                                          \
             strcmp(_R->name_expr.name, node->for_stmt.var) == 0 &&            \
             get_int_literal_value(_L, &_mv) && _mv > 65535))                  \
          _has_overflow2 = 1;                                                  \
      }                                                                        \
      if (_e->type == NODE_BIN_OP && _sp < 30) {                               \
        _stk[_sp++] = _e->bin_op.left;                                         \
        _stk[_sp++] = _e->bin_op.right;                                        \
      }                                                                        \
    }                                                                          \
              ...
              if (_s->type == NODE_ASSIGN)
                _CHK2(_s->assign.value);
              if (_s->type == NODE_SUBSCRIPT_ASSIGN)
                _CHK2(_s->subscript_assign.value);
              if (_s->type == NODE_AUG_ASSIGN)
                _CHK2(_s->aug_assign.value);
#undef _CHK2
            }
            if (!_has_overflow2 &&
                get_int_literal_value(args->items[0], &_s2) &&
                get_int_literal_value(args->items[1], &_e2) && _s2 >= 0 &&
                _e2 <= 1073741824LL)
              _use_int2 = 1;
            if (_use_int2)
          ...
          if (get_int_literal_value(args->items[2], &step_value) &&
              step_value != 0) {
            buf_printf(buf, "for (int64_t lp_%s = ", node->for_stmt.var);
            gen_expr(cg, buf, args->items[0]);
            buf_printf(buf, "; lp_%s %s ", node->for_stmt.var,
                       step_value > 0 ? "<" : ">");
            gen_expr(cg, buf, args->items[1]);
            if (step_value == 1) {
              buf_printf(buf, "; lp_%s++) {\n", node->for_stmt.var);
            ...
            gen_expr(cg, buf, args->items[0]);
            buf_printf(buf, "; lp_%s > ",
                       node->for_stmt.var); /* Dùng dấu > cho step âm */
            gen_expr(cg, buf, args->items[1]);
            buf_printf(buf, "; lp_%s += __lp_step_%d) {\n", node->for_stmt.var,
                       step_id);
      ...
      if (iter_type == LP_NATIVE_ARRAY_1D) {
        ...
        write_indent(buf, indent);
        buf_printf(buf,
                   "for (int64_t __lp_i_%d = 0; __lp_i_%d < __lp_len_%d; "
                   "__lp_i_%d++) {\n",
                   cur_loop, cur_loop, cur_loop, cur_loop);
        write_indent(buf, indent + 1);
        scope_define(cg->scope, node->for_stmt.var, LP_INT);
  ...
  case NODE_PARALLEL_FOR: {
    ...
    const char *schedule =
        node->parallel_for.schedule ? node->parallel_for.schedule : "static";
    ...
    if (num_threads > 0 && chunk_size > 0) {
      buf_printf(
          buf, "#pragma omp parallel for num_threads(%d) schedule(%s, %lld)\n",
    ...
    if (is_range_call(node->parallel_for.iter)) {
      ...
      if (args->count == 1) {
        ...
        if (args->count >= 3 && args->items[2]->type == NODE_UNARY_OP &&
            args->items[2]->unary_op.op == TOK_MINUS &&
            args->items[2]->unary_op.operand->type == NODE_INT_LIT) {
          buf_printf(buf, "; lp_%s > ", node->parallel_for.var);
        } else if (args->count >= 3 && args->items[2]->type == NODE_INT_LIT &&
                   args->items[2]->int_lit.value < 0) {
          buf_printf(buf, "; lp_%s > ", node->parallel_for.var);
        } else if (args->count >= 3) {
          buf_printf(buf, "; lp_%s < ", node->parallel_for.var);
        } else {
      ...
      if (iter_type == LP_NATIVE_ARRAY_1D) {
        ...
        write_indent(buf, indent);
        buf_printf(buf,
                   "for (int64_t __lp_i_%d = 0; __lp_i_%d < __lp_p_len_%d; "
                   "__lp_i_%d++) {\n",
                   cur_loop, cur_loop, cur_loop, cur_loop);
        write_indent(buf, indent + 1);
        scope_define(cg->scope, node->parallel_for.var, LP_INT);
        ...
        buf_printf(buf, "double lp_%s = __lp_p_iter_%d.data[__lp_i_%d];\n",
                 ...
                 iter_type == LP_PYOBJ || iter_type == LP_ARRAY ||
                 iter_type == LP_STR_ARRAY || iter_type == LP_DICT ||
                 iter_type == LP_SET || iter_type == LP_TUPLE) {
        /* Create a unique loop index variable for generic loops to avoid
         * shadowing */
        ...
        /* For OpenMP threads, the loop variable must be explicitly declared
         * locally to be private */
        buf_printf(
            buf,
  ...
  case NODE_TRY: {
    ...
    write_indent(buf, indent + 1);
    buf_write(buf,
              "if (!__lp_exp_caught_local && lp_current_exp.type != 0) {\n");
    write_indent(buf, indent + 2);
  ...
  case NODE_CONST_DECL: {
    ...
    write_indent(buf, indent);
    buf_printf(buf, "const %s lp_%s = ", lp_type_to_c(t),
    ...
    for (int i = 0; i < node->match_stmt.cases.count; i++) {
      ...
      if (case_node->match_case.is_wildcard) {
        ...
        buf_write(buf, "{ /* default case */\n");
                 ...
                 !scope_lookup(old_scope,
                               case_node->match_case.pattern->name_expr.name)) {
        /* Capture variable: bind _match_val to the name, then check guard */
        LpType match_val_type = infer_type(cg, node->match_stmt.value);
        if (match_val_type == LP_UNKNOWN)
          match_val_type = LP_INT;
        const char *cap_name = case_node->match_case.pattern->name_expr.name;
        scope_define(cg->scope, cap_name, match_val_type);

        /* Emit: if (1) { __auto_type lp_n = _match_val; if (guard) { ... } else
           goto next_case; } To keep things simple and integrate with the outer
           `for` body generation without breaking `} else if`, we can generate a
           block that executes conditionally. But C `if-else` chaining doesn't
           like intermediate `}` closures if we want to continue. A cleaner way:
             if ( { __auto_type lp_n = _match_val; (guard) } )
           But statement expressions ({}) evaluate to a value on GCC, which is
           perfect! `if ( ({ __auto_type lp_n = _match_val; guard_expr; }) ) {`
        */
...
static void scan_declarations(CodeGen *cg, NodeList *body) {
  ...
  for (int i = 0; i < body->count; i++) {
    ...
    if (stmt->type == NODE_ASSIGN) {
      ...
      /* If annotated as f32[], keep that type for [0.0]*N pattern */
      if (t == LP_NATIVE_ARRAY_F32_1D && stmt->assign.value)
        match_native_float_repeat_expr(cg, stmt->assign.value, &auto_array_elem,
                                       &auto_array_count);
      /* If annotated as i32[], keep that type for [0]*N pattern */
      if (t == LP_NATIVE_ARRAY_I32_1D && stmt->assign.value)
        match_native_int_repeat_expr(cg, stmt->assign.value, &auto_array_elem,
                                     &auto_array_count);
      /* If annotated as i8[], keep that type for [val]*N pattern */
      if (t == LP_NATIVE_ARRAY_I8_1D && stmt->assign.value)
        match_native_int_repeat_expr(cg, stmt->assign.value, &auto_array_elem,
                                     &auto_array_count);
      if ((t == LP_LIST || t == LP_UNKNOWN || t == LP_VAL) &&
          stmt->assign.value &&
      ...
      /* Auto-promote list annotation to native int array for small all-int
       * literals */
      if (t == LP_LIST && stmt->assign.value &&
          list_expr_all_ints(stmt->assign.value)) {
        t = LP_NATIVE_ARRAY_1D;
      }
      /* Also detect [0.0] * N -> LpFloatArray */
      if ((t == LP_LIST || t == LP_UNKNOWN || t == LP_VAL) &&
          stmt->assign.value &&
          match_native_float_repeat_expr(cg, stmt->assign.value,
                                         &auto_array_elem, &auto_array_count)) {
        t = LP_NATIVE_ARRAY_FLOAT_1D;
      }
      ...
      if ((t == LP_NATIVE_ARRAY_1D || t == LP_LIST) && auto_array_elem &&
          auto_array_count) {
        int64_t init_val = 0;
        ...
        if ((init_val == 0 || init_val == 1) && array_size_hint >= 1000) {
          ...
          for (int bi = 0; bi < body->count && all_bool; bi++) {
            ...
            if (bs->type == NODE_SUBSCRIPT_ASSIGN &&
                ...
                strcmp(bs->subscript_assign.obj->name_expr.name,
                       stmt->assign.name) == 0) {
          ...
          for (int bi = 0; bi < body->count && all_bool; bi++) {
            ...
            NodeList *inner = NULL;
            if (bs->type == NODE_FOR)
              inner = &bs->for_stmt.body;
            if (bs->type == NODE_WHILE)
              inner = &bs->while_stmt.body;
            if (!inner)
              continue;
            for (int ci = 0; ci < inner->count && all_bool; ci++) {
              ...
              if (cs->type == NODE_SUBSCRIPT_ASSIGN &&
                  ...
                  strcmp(cs->subscript_assign.obj->name_expr.name,
                         stmt->assign.name) == 0) {
      ...
      /* ── Fix 3: auto-promote large int arrays to i32[] when values are
       * bounded. Trigger: array init is [0]*N or [-1]*N AND all subsequent
       * assignments have right-hand values that are: result of (expr % K) for
       * K≤65535, or simple counter increments, or index values. This covers LCS
       * dp[] and BFS dist[]/q[] patterns. i32 = 4 bytes vs i64 = 8 bytes → 2×
       * less memory → better L3 cache use. */
      if (t == LP_NATIVE_ARRAY_1D && auto_array_count) {
        /* Only promote if init element is a KNOWN literal (0, -1, or
         * const_int_value). If init is a variable like INF=10^9 we can't safely
         * convert to i32 (the init value itself is fine but the emit path needs
         * repeat with i32 cast). */
        int64_t init_v = 0;
            ...
            if (bs->type == NODE_SUBSCRIPT_ASSIGN &&
              ...
              if (val->type == NODE_INT_LIT) {
                if (val->int_lit.value < -2000000000LL ||
                    val->int_lit.value > 2000000000LL)
                  can_be_i32 = 0;
                ...
                if (val->bin_op.op == TOK_PERCENT &&
                  ...
                  /* i * small_K: safe if K <= 1024 (result <= n*1024 <
                   * INT32_MAX) */
                  int64_t _kmul = 0;
                  if ((val->bin_op.right->type == NODE_INT_LIT &&
                       ((_kmul = val->bin_op.right->int_lit.value) >= 0) &&
                       _kmul <= 1024) ||
                      (val->bin_op.left->type == NODE_INT_LIT &&
                       ((_kmul = val->bin_op.left->int_lit.value) >= 0) &&
                       _kmul <= 1024))
            ...
            for (int ci = 0; ci < l1->count && can_be_i32; ci++) {
              ...
              if (cs->type == NODE_SUBSCRIPT_ASSIGN &&
                ...
                if (val->type == NODE_INT_LIT) {
                  ...
                  int64_t _km = 0;
                  int _ok_op = (val->bin_op.op == TOK_PLUS ||
                                val->bin_op.op == TOK_MINUS ||
                                val->bin_op.op == TOK_PERCENT);
                  if (!_ok_op && val->bin_op.op == TOK_STAR) {
                    if ((val->bin_op.right->type == NODE_INT_LIT &&
                         ((_km = val->bin_op.right->int_lit.value) >= 0) &&
                         _km <= 1024) ||
                        (val->bin_op.left->type == NODE_INT_LIT &&
                         ((_km = val->bin_op.left->int_lit.value) >= 0) &&
                         _km <= 1024))
                      _ok_op = 1;
                  }
                  if (!_ok_op)
              ...
              if (cs->type == NODE_FOR)
                l2 = &cs->for_stmt.body;
              if (cs->type == NODE_WHILE)
                l2 = &cs->while_stmt.body;
              if (cs->type == NODE_IF)
                l2 = &cs->if_stmt.then_body;
              if (!l2)
                continue;
              for (int di = 0; di < l2->count && can_be_i32; di++) {
                ...
                if (ds->type == NODE_FOR)
                  l3 = &ds->for_stmt.body;
                if (ds->type == NODE_WHILE)
                  l3 = &ds->while_stmt.body;
                if (ds->type == NODE_IF)
                  l3 = &ds->if_stmt.then_body;
                if (!l3)
                  continue;
                for (int ei = 0; ei < l3->count && can_be_i32; ei++) {
                  ...
                  if (es->type == NODE_SUBSCRIPT_ASSIGN &&
                      ...
                      strcmp(es->subscript_assign.obj->name_expr.name,
                             stmt->assign.name) == 0) {
       ...
       * propagation. Condition: assigned from a positive int literal AND never
       * reassigned at ANY nesting level (including inside for/while/if bodies).
       */
      if ((t == LP_INT) && stmt->assign.value &&
        ...
        for (int ri = 0; ri < body->count && !reassigned; ri++) {
          ...
          if (rs->type == NODE_ASSIGN && rs != stmt &&
              strcmp(rs->assign.name, vname) == 0)
            reassigned = 1;
          if (rs->type == NODE_AUG_ASSIGN &&
              strcmp(rs->aug_assign.name, vname) == 0)
            reassigned = 1;
          /* Check inside for/while bodies */
          NodeList *inner = NULL;
          if (rs->type == NODE_FOR)
            inner = &rs->for_stmt.body;
          if (rs->type == NODE_WHILE)
            inner = &rs->while_stmt.body;
          if (inner) {
            for (int ii = 0; ii < inner->count && !reassigned; ii++) {
              AstNode *is = inner->items[ii];
              if (is->type == NODE_ASSIGN &&
                  strcmp(is->assign.name, vname) == 0)
                reassigned = 1;
              if (is->type == NODE_AUG_ASSIGN &&
                  strcmp(is->aug_assign.name, vname) == 0)
                reassigned = 1;
              /* One more level deep */
              NodeList *inner2 = NULL;
              if (is->type == NODE_FOR)
                inner2 = &is->for_stmt.body;
              if (is->type == NODE_WHILE)
                inner2 = &is->while_stmt.body;
              if (inner2) {
                for (int ji = 0; ji < inner2->count && !reassigned; ji++) {
                  AstNode *js = inner2->items[ji];
                  if (js->type == NODE_ASSIGN &&
                      strcmp(js->assign.name, vname) == 0)
                    reassigned = 1;
                  if (js->type == NODE_AUG_ASSIGN &&
                      strcmp(js->aug_assign.name, vname) == 0)
                    reassigned = 1;
                }
              }
            }
       ...
       * We scan the for-body; any LP_NATIVE_ARRAY_1D assigned there gets
       * hoisted to the outer scope (current body scope). The for-body will see
       * it as "already declared" and emit lp_int_array_reuse() instead of
       * lp_int_array_new(). */
      for (int bi = 0; bi < stmt->for_stmt.body.count; bi++) {
        ...
        if (bs->type == NODE_ASSIGN) {
          ...
          if (bs->assign.type_ann)
            bt = type_from_annotation(cg, bs->assign.type_ann);
          else if (bs->assign.value)
            bt = infer_type(cg, bs->assign.value);
          ...
          if (bt == LP_NATIVE_ARRAY_1D && cnt2) {
            ...
            if (!outer) {
              ...
              outer =
                  scope_define(cg->scope, bs->assign.name, LP_NATIVE_ARRAY_1D);
              if (outer)
                outer->is_loop_hoisted = 1;
            }
          }
        }
      }
...
static void emit_local_declarations(CodeGen *cg, Buffer *buf, Scope *scope,
                                    int indent) {
  if (!scope)
    return;
  ...
  for (int i = 0; i < scope->count; i++) {
    ...
    /* Skip functions, classes, native arrays (unless hoisted), and already
     * declared vars */
    if (!sym->is_function && sym->type != LP_CLASS && !sym->declared) {
      int is_native_arr =
          (sym->type == LP_NATIVE_ARRAY_1D || sym->type == LP_NATIVE_ARRAY_2D ||
           sym->type == LP_NATIVE_ARRAY_FLOAT_1D ||
           sym->type == LP_NATIVE_ARRAY_FLOAT_2D ||
           sym->type == LP_NATIVE_ARRAY_I32_1D ||
           sym->type == LP_NATIVE_ARRAY_I32_2D ||
           sym->type == LP_NATIVE_ARRAY_F32_1D ||
           sym->type == LP_NATIVE_ARRAY_F32_2D ||
      ...
      if (sym->is_loop_hoisted && sym->type == LP_NATIVE_ARRAY_1D) {
         ...
         * gen_stmt will see declared=0 inside the loop and emit
         * lp_int_array_new() on first iteration, then lp_int_array_reuse() on
         * subsequent iterations. Actually: mark declared=1 here so gen_stmt
         * uses lp_int_array_reuse path. The first call to
         * lp_int_array_reuse(NULL, N) allocates (NULL check inside). */
        write_indent(buf, indent);
        ...
        buf_printf(buf, "%s lp_%s;\n", lp_type_to_c_obj(sym->type, class_name),
                   sym->name);
        sym->declared = 1;
      }
    }
...
static void populate_function_symbol(CodeGen *cg, Symbol *sym, AstNode *node,
                                     const char *owner_class) {
  if (!sym || !node || node->type != NODE_FUNC_DEF)
    return;
  ...
  for (int i = 0; i < node->func_def.params.count; i++) {
    ...
    if (sym->first_param_type == LP_UNKNOWN) {
      ...
      if (pt == LP_OBJECT) {
        const char *class_name = owner_class && strcmp(p->name, "self") == 0
                                     ? owner_class
                                     : p->type_ann;
        if (class_name)
          sym->first_param_class_name = strdup(class_name);
      }
    }
  }
}
...
static int expr_has_self_call(AstNode *e, const char *fname) {
  ...
  if (e->type == NODE_CALL && e->call.func->type == NODE_NAME &&
      strcmp(e->call.func->name_expr.name, fname) == 0)
    return 1;
  /* recurse into sub-expressions */
  if (e->type == NODE_BIN_OP)
    return expr_has_self_call(e->bin_op.left, fname) ||
           expr_has_self_call(e->bin_op.right, fname);
  if (e->type == NODE_UNARY_OP)
    return expr_has_self_call(e->unary_op.operand, fname);
  if (e->type == NODE_CALL) {
    for (int i = 0; i < e->call.args.count; i++)
      if (expr_has_self_call(e->call.args.items[i], fname))
        return 1;
  }
  return 0;
}
static int body_has_self_call(NodeList *body, const char *fname) {
  ...
  for (int i = 0; i < body->count; i++) {
    ...
    if (s->type == NODE_RETURN &&
        expr_has_self_call(s->return_stmt.value, fname))
      return 1;
    if (s->type == NODE_ASSIGN && expr_has_self_call(s->assign.value, fname))
      return 1;
    if (s->type == NODE_EXPR_STMT &&
        expr_has_self_call(s->expr_stmt.expr, fname))
      return 1;
    if (s->type == NODE_IF) {
      if (body_has_self_call(&s->if_stmt.then_body, fname))
        return 1;
    }
  }
  return 0;
...
static int body_needs_avx(NodeList *body) {
  ...
  for (int i = 0; i < body->count; i++) {
    ...
    if (s->type == NODE_ASSIGN && s->assign.value) {
      ...
      if (v->type == NODE_FLOAT_LIT)
        return 1;
      if (v->type == NODE_CALL && v->call.func->type == NODE_ATTRIBUTE)
        return 1;
    }
  }
  return 0;
...
static void gen_func_def(CodeGen *cg, AstNode *node, const char *class_name) {
  ...
  buf_printf(&cg->header, "static inline %s lp_%s(", lp_type_to_c(ret),
             node->func_def.name);
  for (int i = 0; i < node->func_def.params.count; i++) {
    Param *p = &node->func_def.params.items[i];
    if (i > 0)
   ...
   * access to all AVX-512 integer instructions and better instruction
   * selection. */
  int is_pure_int_recursive =
      (ret == LP_INT || ret == LP_VOID) &&
  ...
  if (is_pure_int_recursive) {
     ...
     * Result: matches or beats C++ at identical algorithm (tested 0.197 vs
     * 0.209) */
      ...
      if (pt == LP_OBJECT) {
        ...
        if (sym)
          sym->declared = 1;
                 ...
                 pt == LP_NATIVE_ARRAY_FLOAT_1D ||
                 pt == LP_NATIVE_ARRAY_FLOAT_2D ||
                 pt == LP_NATIVE_ARRAY_I32_1D || pt == LP_NATIVE_ARRAY_I32_2D ||
                 pt == LP_NATIVE_ARRAY_F32_1D || pt == LP_NATIVE_ARRAY_F32_2D ||
                 pt == LP_NATIVE_ARRAY_I8_1D) {
        /* __restrict__: no aliasing between native array params -> enables
         * vectorization */
        ...
        if (sym) {
          ...
          if (pt == LP_INT) {
            ...
            for (int _pi = 0, _ri = 0; _pi < node->func_def.params.count;
                 _pi++) {
              Param *_pp = &node->func_def.params.items[_pi];
              if (_pp->is_vararg || _pp->is_kwarg)
                continue;
              if (strcmp(_pp->name, p->name) == 0) {
                pi = _ri;
                break;
              }
              _ri++;
   ...
   * and / */
  /* SAFETY: only for NON-recursive functions. Recursive fns call with n-1/n-2
   * which reach 0. */
  if (!is_pure_int_recursive) {
    Symbol *fn_sym2 = scope_lookup(
        cg->scope->parent ? cg->scope->parent : cg->scope, node->func_def.name);
    if (fn_sym2 && fn_sym2->non_negative_param_mask) {
      int real_idx = 0;
      for (int _pi = 0; _pi < node->func_def.params.count; _pi++) {
        Param *_pp = &node->func_def.params.items[_pi];
        if (_pp->is_vararg || _pp->is_kwarg || !_pp->name)
          continue;
  ...
  /* NOTE: _raw_ pointer unwraps are emitted per-function after local var
   * declarations */
   ...
   * Macros are always current regardless of allocation order. */
  {
    ...
    for (int _si = 0; _si < func_scope->count; _si++) {
      ...
      if (_pt == LP_NATIVE_ARRAY_1D || _pt == LP_NATIVE_ARRAY_2D ||
          ...
          _pt == LP_NATIVE_ARRAY_I8_1D) {
        /* Emit _raw_ macro with assume_aligned for AVX2 aligned loads */
        {
          const char *elem_t2 = "void";
          if (_pt == LP_NATIVE_ARRAY_1D || _pt == LP_NATIVE_ARRAY_2D)
            elem_t2 = "int64_t";
          else if (_pt == LP_NATIVE_ARRAY_FLOAT_1D ||
                   _pt == LP_NATIVE_ARRAY_FLOAT_2D)
            elem_t2 = "double";
          else if (_pt == LP_NATIVE_ARRAY_I32_1D ||
                   _pt == LP_NATIVE_ARRAY_I32_2D)
            elem_t2 = "int32_t";
          else if (_pt == LP_NATIVE_ARRAY_F32_1D ||
                   _pt == LP_NATIVE_ARRAY_F32_2D)
            elem_t2 = "float";
          else if (_pt == LP_NATIVE_ARRAY_I8_1D)
            elem_t2 = "int8_t";
          buf_printf(
              &cg->funcs,
              "    #define _raw_%s ((%s* "
  ...
  for (int d = 0; d < node->func_def.decorators.count; d++) {
    ...
    if (decorator->type == NODE_SETTINGS && decorator->settings.enabled) {
      ...
      if (decorator->settings.schedule) {
        buf_printf(
            &cg->funcs, "    lp_parallel_configure(%d, \"%s\", %lld, 0, 0);\n",
            decorator->settings.num_threads, decorator->settings.schedule,
  ...
  for (int d = 0; d < node->func_def.decorators.count; d++) {
    ...
    if (decorator->type == NODE_SECURITY && decorator->security.enabled) {
      ...
      if (decorator->security.rate_limit > 0) {
        buf_printf(&cg->funcs,
                   "    if (!lp_check_func_rate_limit(\"%s\", %d)) {\n",
                   node->func_def.name, decorator->security.rate_limit);
        buf_printf(&cg->funcs,
                   "        fprintf(stderr, \"[SECURITY] Rate limit exceeded "
                   "for %s\\n\");\n",
                   node->func_def.name);
      ...
      if (decorator->security.require_auth) {
        ...
        buf_printf(&cg->funcs,
                   "        fprintf(stderr, \"[SECURITY] Authentication "
                   "required for %s\\n\");\n",
                   node->func_def.name);
        buf_write(&cg->funcs, "        return 0;\n");
        buf_write(&cg->funcs, "    }\n");
      ...
      if (required_level > 0) {
        ...
        buf_write(&cg->funcs, ") {\n");
        buf_printf(&cg->funcs,
                   "        fprintf(stderr, \"[SECURITY] Access denied for %s "
      ...
      if (decorator->security.readonly) {
        buf_write(
            &cg->funcs,
            "    /* Readonly mode: write operations will be blocked */\n");
      }
 ...
 * The 'async' keyword serves as syntactic sugar for documentation/annotation
 * purposes. 'await' expressions inside async functions simply evaluate
 * synchronously.
 *
 ...
 */
static void gen_async_func_def(CodeGen *cg, AstNode *node,
                               const char *class_name) {
  ...
  if (func_sym) {
    ...
    if (node->async_def.ret_type) {
      ...
      cg->scope = temp_scope->parent;
      scope_free(temp_scope);
      if (func_sym->type == LP_UNKNOWN)
        func_sym->type = LP_VOID;
    }
  ...
  buf_printf(&cg->header, "/* async */ %s lp_%s(", lp_type_to_c(ret),
             node->async_def.name);
  for (int i = 0; i < node->async_def.params.count; i++) {
    Param *p = &node->async_def.params.items[i];
    if (i > 0)
...
static void gen_class_def(CodeGen *cg, AstNode *node) {
  ...
  if (base_class) {
    for (Scope *sc = cg->scope; sc != NULL; sc = sc->parent) {
      ...
      for (int i = 0; i < initial_count; i++) {
        ...
        if (strncmp(sym->name, base_class, blen) == 0 &&
            sym->name[blen] == '.') {
          char new_key[256];
          ...
          if (dot) {
            ...
            if (nsym) {
              ...
              nsym->owner_class =
                  sym->owner_class ? strdup(sym->owner_class) : NULL;
              ...
              if (sym->first_param_class_name) {
                nsym->first_param_class_name =
                    strdup(sym->first_param_class_name);
              }
            }
            if (!sym->is_method) {
  ...
  for (int i = 0; i < node->class_def.body.count; i++) {
    ...
    if (stmt->type == NODE_ASSIGN) {
      LpType t = stmt->assign.type_ann
                     ? type_from_annotation(cg, stmt->assign.type_ann)
                     : infer_type(cg, stmt->assign.value);
      if (t == LP_UNKNOWN)
        t = LP_INT;
      buf_printf(&struct_def, "    %s lp_%s;\n", lp_type_to_c(t),
                 stmt->assign.name);
      ...
      char field_key[256];
      snprintf(field_key, sizeof(field_key), "%s.%s", class_name,
               stmt->assign.name);
  ...
  for (int i = 0; i < node->class_def.body.count; i++) {
    ...
    if (stmt->type == NODE_FUNC_DEF) {
      ...
      char method_key[256];
      snprintf(method_key, sizeof(method_key), "%s.%s", class_name,
               stmt->func_def.name);
      LpType rt = stmt->func_def.ret_type
                      ? type_from_annotation(cg, stmt->func_def.ret_type)
                      : LP_VOID;
      ...
      snprintf(method_name, sizeof(method_name), "%s_%s", class_name,
               stmt->func_def.name);
...
void codegen_generate(CodeGen *cg, AstNode *program) {
  ...
  for (int i = 0; i < program->program.stmts.count; i++) {
    ...
    if (stmt->type == NODE_IMPORT) {
      ...
      if (!module) {
        codegen_set_error(
            cg, "Encountered invalid import module (OOM during parse)");
        continue; // BỎ QUA NẾU MODULE BỊ NULL
      }
      const char *alias =
          stmt->import_stmt.alias ? stmt->import_stmt.alias : module;
      ...
      if (strcmp(module, "math") == 0)
        ...
        tier = MOD_TIER1_RANDOM;
      else if (strcmp(module, "time") == 0)
        tier = MOD_TIER1_TIME;
      else if (strcmp(module, "numpy") == 0)
        tier = MOD_TIER2_NUMPY;
      else if (strcmp(module, "os") == 0)
        tier = MOD_TIER1_OS;
      else if (strcmp(module, "sys") == 0)
        tier = MOD_TIER1_SYS;
      else if (strcmp(module, "string") == 0)
        tier = MOD_TIER1_STRING;
      else if (strcmp(module, "http") == 0)
        tier = MOD_TIER1_HTTP;
      else if (strcmp(module, "json") == 0)
        tier = MOD_TIER1_JSON;
      else if (strcmp(module, "sqlite") == 0)
        tier = MOD_TIER1_SQLITE;
      else if (strcmp(module, "thread") == 0)
        tier = MOD_TIER1_THREAD;
      else if (strcmp(module, "memory") == 0)
        tier = MOD_TIER1_MEMORY;
      else if (strcmp(module, "platform") == 0)
        tier = MOD_TIER1_PLATFORM;
      else if (strcmp(module, "security") == 0)
        tier = MOD_TIER1_SECURITY;
      else if (strcmp(module, "dsa") == 0)
        tier = MOD_TIER1_DSA;
      else if (strcmp(module, "regex") == 0)
        tier = MOD_TIER1_REGEX;
      else if (strcmp(module, "hashlib") == 0)
        tier = MOD_TIER1_HASHLIB;
      else if (strcmp(module, "base64") == 0)
        tier = MOD_TIER1_BASE64;
      else if (strcmp(module, "csv") == 0)
        tier = MOD_TIER1_CSV;
      else if (strcmp(module, "datetime") == 0)
        tier = MOD_TIER1_DATETIME;
      else if (strcmp(module, "socket") == 0)
        tier = MOD_TIER1_SOCKET;
      else if (strcmp(module, "logging") == 0)
        tier = MOD_TIER1_LOGGING;
      else if (strcmp(module, "unittest") == 0)
      ...
      if (cg->import_count < 64) {
        ...
        if (mod_dup && alias_dup) {
          ...
          if (tier == MOD_TIER3_PYTHON)
            cg->uses_python = 1;
          else if (tier == MOD_TIER1_OS) {
            cg->uses_native = 1;
            cg->uses_os = 1;
          } else if (tier == MOD_TIER1_SYS) {
            cg->uses_native = 1;
            cg->uses_sys = 1;
          } else if (tier == MOD_TIER1_STRING) {
            cg->uses_native = 1;
            cg->uses_strings = 1;
          } else if (tier == MOD_TIER1_HTTP) {
            cg->uses_native = 1;
            cg->uses_http = 1;
          } else if (tier == MOD_TIER1_JSON) {
            cg->uses_native = 1;
            cg->uses_json = 1;
          } else if (tier == MOD_TIER1_SQLITE) {
            cg->uses_native = 1;
            cg->uses_sqlite = 1;
          } else if (tier == MOD_TIER1_THREAD) {
            cg->uses_native = 1;
            cg->uses_thread = 1;
          } else if (tier == MOD_TIER1_MEMORY) {
            cg->uses_native = 1;
            cg->uses_memory = 1;
          } else if (tier == MOD_TIER1_PLATFORM) {
            cg->uses_native = 1;
            cg->uses_platform = 1;
          } else if (tier == MOD_TIER1_SECURITY) {
            cg->uses_native = 1;
            cg->uses_security = 1;
          } else if (tier == MOD_TIER1_DSA) {
            cg->uses_native = 1;
            cg->uses_dsa = 1;
          } else if (tier == MOD_TIER1_REGEX) {
            cg->uses_native = 1;
            cg->uses_regex = 1;
          } else if (tier == MOD_TIER1_HASHLIB) {
            cg->uses_native = 1;
            cg->uses_hashlib = 1;
          } else if (tier == MOD_TIER1_BASE64) {
            cg->uses_native = 1;
            cg->uses_base64 = 1;
          } else if (tier == MOD_TIER1_CSV) {
            cg->uses_native = 1;
            cg->uses_csv = 1;
          } else if (tier == MOD_TIER1_DATETIME) {
            cg->uses_native = 1;
            cg->uses_datetime = 1;
          } else if (tier == MOD_TIER1_SOCKET) {
            cg->uses_native = 1;
            cg->uses_socket = 1;
          } else if (tier == MOD_TIER1_LOGGING) {
            cg->uses_native = 1;
            cg->uses_logging = 1;
          } else if (tier == MOD_TIER1_UNITTEST) {
            cg->uses_native = 1;
            cg->uses_unittest = 1;
  ...
  if (cg->uses_regex)
    buf_write(&cg->header, "#include \"lp_regex.h\"\n");
  if (cg->uses_hashlib)
    buf_write(&cg->header, "#include \"lp_hashlib.h\"\n");
  if (cg->uses_base64)
    buf_write(&cg->header, "#include \"lp_base64.h\"\n");
  if (cg->uses_csv)
    buf_write(&cg->header, "#include \"lp_csv.h\"\n");
  if (cg->uses_datetime)
    buf_write(&cg->header, "#include \"lp_datetime.h\"\n");
  if (cg->uses_socket)
    buf_write(&cg->header, "#include \"lp_socket.h\"\n");
  if (cg->uses_logging)
    buf_write(&cg->header, "#include \"lp_logging.h\"\n");
  if (cg->uses_unittest)
  ...
  /* Second pass: forward register all function symbols (including async
   * functions) */
  for (int i = 0; i < program->program.stmts.count; i++) {
    AstNode *stmt = program->program.stmts.items[i];
    if (stmt->type == NODE_FUNC_DEF) {
   ...
   * lp_mod/lp_floordiv in called functions. */
  for (int i = 0; i < program->program.stmts.count; i++) {
    AstNode *stmt = program->program.stmts.items[i];
    ...
    if (stmt->type == NODE_FUNC_DEF) {
      ...
      for (int j = 0; j < body->count; j++) {
        ...
        if (bs->type != NODE_FOR)
          continue;
        if (!is_range_call(bs->for_stmt.iter))
          continue;
        ...
        for (int k = 0; k < bs->for_stmt.body.count; k++) {
          ...
          if (!call_node)
            continue;
          if (!call_node->call.func || call_node->call.func->type != NODE_NAME)
            continue;
          ...
          for (int ai = 0; ai < call_node->call.args.count && ai < 32; ai++) {
            ...
            if (arg->type == NODE_NAME &&
                strcmp(arg->name_expr.name, loop_var) == 0)
              arg_nn = 1;
            /* Arg is a positive literal */
            int64_t av = 0;
            if (get_int_literal_value(arg, &av) && av >= 0)
              arg_nn = 1;
            if (arg_nn)
  ...
  for (int i = 0; i < program->program.stmts.count; i++) {
    ...
    if (stmt->type == NODE_FUNC_DEF) {
      ...
      /* Skip bare top-level main() call: codegen already emits lp_main() in C
       * main() */
      int skip = 0;
      if (cg->has_main && stmt->type == NODE_EXPR_STMT) {
        AstNode *expr = stmt->expr_stmt.expr;
        if (expr && expr->type == NODE_CALL && expr->call.func &&
            expr->call.func->type == NODE_NAME &&
...
void codegen_generate_header(CodeGen *cg, AstNode *program,
  ...
  if (!f) {
    snprintf(cg->error_msg, sizeof(cg->error_msg),
             "Cannot open header path %s for writing", header_path);
    cg->had_error = 1;
    return;
  }
  ...
  for (int i = 0; i < program->program.stmts.count; i++) {
    AstNode *stmt = program->program.stmts.items[i];
    if (stmt->type == NODE_FUNC_DEF) {
      if (stmt->func_def.access == TOK_PRIVATE)
        continue;
      if (stmt->func_def.name[0] == '_')
        continue; /* By convention, _ means private */
      ...
      for (int k = 0; k < stmt->func_def.params.count; k++) {
        ...
        LpType pt = stmt->func_def.params.items[k].type_ann
                        ? type_from_annotation(
                              cg, stmt->func_def.params.items[k].type_ann)
                        : LP_FLOAT;
        fprintf(f, "%s lp_%s", lp_type_to_c(pt),
                stmt->func_def.params.items[k].name);
      }
...
char *codegen_get_output(CodeGen *cg) {
  ...
  if (cg->uses_parallel)
    buf_write(&out, "#include \"lp_parallel.h\"\n");
  if (cg->uses_gpu)
