...
#include <string.h>
  ...
  /* 
   * Generic HTTP request method for PUT, DELETE, PATCH
   * Security: Run curl with fork/exec
   */
  static inline char* lp_curl_request(const char *url, const char *method, const char *data) {
      int pipefd[2];
      int data_pipe[2];
      pid_t pid;
      ...
      /* Create pipe for request data if needed */
      if (data && pipe(data_pipe) < 0) {
          close(pipefd[0]);
          close(pipefd[1]);
          return strdup("");
      ...
      if (pid == 0) {
          ...
          if (data) {
              ...
              const char *curl_args[] = {"curl", "-s", "-L", "-X", method, 
                                         "-d", "@-", url, NULL};
              execvp("curl", (char *const *)curl_args);
          } else {
              const char *curl_args[] = {"curl", "-s", "-L", "-X", method, url, NULL};
              execvp("curl", (char *const *)curl_args);
          }
...
/* lp_http_put(url, data) - HTTP PUT request */
static inline char* lp_http_put(const char *url, const char *data) {
#if defined(_WIN32)
    /* Windows implementation similar to POST but with PUT method */
    HINTERNET hSession = NULL, hConnect = NULL, hRequest = NULL;
    URL_COMPONENTS urlComp;
    memset(&urlComp, 0, sizeof(urlComp));
    urlComp.dwStructSize = sizeof(urlComp);
    ...
    hSession = WinHttpOpen(L"LP HTTP/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
    ...
    if (hConnect) {
        DWORD flags = (urlComp.nScheme == INTERNET_SCHEME_HTTPS) ? WINHTTP_FLAG_SECURE : 0;
        hRequest = WinHttpOpenRequest(hConnect, L"PUT", urlPath, NULL, WINHTTP_NO_REFERER,
                                       WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
        if (hRequest) {
            LPCWSTR header = L"Content-Type: application/json\r\n";
            WinHttpAddRequestHeaders(hRequest, header, (ULONG)-1L, WINHTTP_ADDREQ_FLAG_ADD);
            DWORD data_len = data ? (DWORD)strlen(data) : 0;
            if (WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, (LPVOID)data, data_len, data_len, 0) &&
    ...
    return buffer;
#else
    return lp_curl_request(url, "PUT", data);
#endif
}

/* lp_http_delete(url) - HTTP DELETE request */
static inline char* lp_http_delete(const char *url) {
#if defined(_WIN32)
    HINTERNET hSession = NULL, hConnect = NULL, hRequest = NULL;
    URL_COMPONENTS urlComp;
    memset(&urlComp, 0, sizeof(urlComp));
    ...
    hSession = WinHttpOpen(L"LP HTTP/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                           WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (hSession) hConnect = WinHttpConnect(hSession, hostName, urlComp.nPort, 0);
    
    char *buffer = strdup("");
    if (hConnect) {
        DWORD flags = (urlComp.nScheme == INTERNET_SCHEME_HTTPS) ? WINHTTP_FLAG_SECURE : 0;
        hRequest = WinHttpOpenRequest(hConnect, L"DELETE", urlPath, NULL, WINHTTP_NO_REFERER,
                                       WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
        if (hRequest) {
    ...
    return buffer;
#else
    return lp_curl_request(url, "DELETE", NULL);
#endif
}

/* lp_http_patch(url, data) - HTTP PATCH request */
static inline char* lp_http_patch(const char *url, const char *data) {
#if defined(_WIN32)
    HINTERNET hSession = NULL, hConnect = NULL, hRequest = NULL;
    URL_COMPONENTS urlComp;
    memset(&urlComp, 0, sizeof(urlComp));
    ...
    if (hConnect) {
        DWORD flags = (urlComp.nScheme == INTERNET_SCHEME_HTTPS) ? WINHTTP_FLAG_SECURE : 0;
        hRequest = WinHttpOpenRequest(hConnect, L"PATCH", urlPath, NULL, WINHTTP_NO_REFERER,
                                       WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
        if (hRequest) {
            LPCWSTR header = L"Content-Type: application/json\r\n";
            WinHttpAddRequestHeaders(hRequest, header, (ULONG)-1L, WINHTTP_ADDREQ_FLAG_ADD);
    ...
    return buffer;
#else
    return lp_curl_request(url, "PATCH", data);
#endif
}
